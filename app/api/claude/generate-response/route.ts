...
export async function POST(req: NextRequest) {
  ...
  let computedAttribution: { modelName?: string; r2?: number; perfLevel?: string; confidence?: string } = {};
        ...
        console.log(`[Claude DEBUG] Determined primaryAnalysisField: ${primaryAnalysisField}`);
          ...
          // For chat requests, skip the field existence check since we're using context-based responses
          if (isContextualChat && primaryAnalysisField === 'chat_context') {
              relevantDataFound = true;
              console.log('[Claude Prompt Gen] Chat context request - bypassing field existence check');
          } else if (primaryAnalysisField && processedLayersData.some(layer =>
...
`;
        ...
        if (hasComprehensiveSummary) {
          ...
          // MODEL ATTRIBUTION REMOVED: No longer showing technical ML details to users
...
Present this analysis in your professional ${selectedPersona.name} style while preserving all data exactly.`;
  ...
  // MODEL ATTRIBUTION REMOVED: No post-processing needed
