...
export class QueryParser {
  ...
  private debugMode =
    ...
    process.env.DEBUG_QUERY_ROUTING === "true";
  ...
  parse(query: string): ParsedQuery {
    ...
    if (this.debugMode && allMatches.length > 0) {
      ...
      console.log(
        `[QueryParser] Top matches for "${normalizedQuery.substring(0, 50)}...": ${topMatches.join(", ")}`,
      );
    }
  ...
  private extractEntities(
    ...
    for (const handler of this.handlers) {
      if (
        ...
        if (
          "extractEntities" in handler &&
          typeof handler.extractEntities === "function"
        ) {
