/**
 * ResponseEnhancer - Implements Principles 5, 10, and 14
 *
 * Principle 5: Adaptive Communication - Adjusts response style based on user preferences
 * Principle 10: Emotional Intelligence - Handles politically sensitive topics appropriately
 * Principle 14: "So What" Framing - Leads with insights, not raw data
 *
 * This enhancer is applied to all handler responses before they're shown to users.
 */
...
export interface EnhancementContext {
  ...
  dataVsInsightRatio: number; // 0 = all insight, 1 = all data
}

// ============================================================================
// Principle 5: Adaptive Communication
// ============================================================================
...
/**
 * Detect user expertise level from session behavior
 */
 ...
 */
export function adaptResponseLength(
  response: string,
...
/**
 * Get auto style based on expertise
 */
...
/**
 * Create a concise version of a response
 * Removes explanatory text, keeps data and actions
 */
...
// ============================================================================
// Principle 10: Emotional Intelligence
// ============================================================================

/**
 * Politically sensitive topic patterns
 */
...
/**
 * Detect political sensitivity level of content
 */
export function detectPoliticalSensitivity(
  query: string,
  response: string
): 'low' | 'medium' | 'high' {
...
/**
 * Close race patterns that need empathetic framing
 */
...
/**
 * Detect if response discusses close/uncertain races
 */
export function detectCloseRace(response: string): boolean {
  return CLOSE_RACE_PATTERNS.some(pattern => pattern.test(response));
}

/**
 * Add emotionally intelligent framing to response
 */
export function addEmotionalIntelligence(
  response: string,
...
// ============================================================================
// Principle 14: "So What" Framing
// ============================================================================

/**
 * Insight patterns - things that should come first
 */
...
/**
 * Data patterns - things that should come after insights
 */
const DATA_PATTERNS = [
...
/**
 * Check if response leads with insight (good) or data (needs reframing)
 */
export function analyzeInsightDataRatio(response: string): number {
...
/**
 * Strategic insight generators for different query types
 */
const STRATEGIC_INSIGHTS: Record<string, (data: Record<string, unknown>) => string> = {
  segment_find: (data) => {
    const count = Number(data.matchCount || data.precinctCount) || 0;
    const voters = Number(data.totalVoters) || 0;
    if (count === 0) return 'No precincts match your criteria - consider broadening your filters.';
...
/**
 * Add "So What" framing if response lacks strategic insight upfront
 */
export function addSoWhatFraming(
  response: string,
  intent: string,
...
/**
 * Enhance a handler result with all three principles
 */
export function enhanceResponse(
  ...
  // Principle 14: Add "So What" framing first (if needed)
...
export default {
  ...
  shouldEnhance,
  getUserPreferences,
  detectPoliticalSensitivity,
  addSoWhatFraming,
  addEmotionalIntelligence,
  adaptResponseLength,
};
