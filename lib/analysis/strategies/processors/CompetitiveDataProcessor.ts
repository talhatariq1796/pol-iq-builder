...
export class CompetitiveDataProcessor implements DataProcessorStrategy {
      ...
      rawData.results.some((r) => {
        ...
        return (
          ...
          ((record as any).area_id ||
            (record as any).id ||
            (record as any).ID) &&
          ((record as any).competitive_score !== undefined ||
            (record as any).competitive_analysis_score !== undefined ||
  ...
  process(rawData: RawAnalysisResult): ProcessedAnalysisData {
    console.log(
      `🔥🔥🔥 [COMPETITIVE PROCESSOR] CALLED WITH ${rawData.results?.length || 0} RECORDS 🔥🔥🔥`,
    );
    console.log(
      `[CompetitiveDataProcessor] Process method called with ${rawData.results?.length || 0} records`,
    );
    ...
    console.log(
      "[CompetitiveDataProcessor] Using primaryField for reading:",
      primaryField,
      "outputting as:",
      targetField,
    );
    ...
    const records = this.processCompetitiveRecords(
      rawData.results,
      primaryField,
    );

    console.log(
      `[CompetitiveDataProcessor] Processed ${records.length} records`,
    );
    console.log(`[CompetitiveDataProcessor] Sample processed record:`, {
      ...
      competitive_advantage_score:
        records[0]?.properties?.competitive_advantage_score,
    });
    ...
    */
    ...
    console.log(
      `🎯 [COMPETITIVE DATA] Filtered ${records.length - nonParkRecords.length} parks from competitive analysis`,
    );
    ...
    const competitiveAnalysis =
      this.analyzeCompetitiveLandscape(nonParkRecords);

    // Process feature importance for competitive factors
    const featureImportance = this.processCompetitiveFeatureImportance(
      rawData.feature_importance || [],
    );

    // Generate competitive summary using filtered records
    const summary = this.generateCompetitiveSummary(
      nonParkRecords,
      competitiveAnalysis,
    );
  ...
  private processCompetitiveRecords(
    rawRecords: any[],
    primaryField: string,
  ): GeographicDataPoint[] {
    return rawRecords
      .map((record, index) => {
        const area_id =
          ...
          (record as any).GEOID ||
          `area_${index}`;
        const area_name = resolveAreaName(record, {
          mode: "zipCity",
          neutralFallback: `Area ${area_id || index + 1}`,
        });

        // Extract competitive metrics
        const competitiveScore = this.extractCompetitiveScore(
          record,
          primaryField,
        );
        ...
        const properties = {
          ...
          ...(primaryField !== "competitive_analysis_score" && {
          ...
          competitor_shares: competitors.map((c) => ({
            name: c.metricName,
            share: c.value,
          })),
          ...
          competitive_position:
            this.determineCompetitivePosition(competitiveScore),
        ...
        const category = this.getCompetitiveCategory(
          competitiveScore,
          marketShare,
        );
  ...
  private extractCompetitiveScore(record: any, primaryField: string): number {
    ...
    const score = Number(
      (record as any)[primaryField] ||
        (record as any).competitive_score ||
        (record as any).competitive_analysis_score ||
        (record as any).competitive_advantage_score,
    );
    ...
    if (isNaN(score)) {
      console.warn(
        `[CompetitiveDataProcessor] No competitive score found for record ${recordId}, calculating composite score`,
      );
    ...
    console.log(
      `🔥 [CompetitiveDataProcessor] Using competitive score: ${score} from ${(record as any).competitive_score ? "competitive_score" : (record as any).competitive_analysis_score ? "competitive_analysis_score" : "competitive_advantage_score"}`,
    );
    return score;
  }
  ...
  private calculateCompositeCompetitiveScore(record: any): number {
    ...
    const population =
      Number((record as any).value_TOTPOP_CY) ||
      Number((record as any).TOTPOP_CY) ||
      0;
    if (population > 0) {
    ...
    const income =
      Number((record as any).value_AVGHINC_CY) ||
      Number((record as any).AVGHINC_CY) ||
      0;
    if (income > 0) {
    ...
    if (strategicScore > 10) {
      // Only use if it's likely a real strategic score
    ...
    const finalScore =
      factorCount > 0 ? (compositeScore / factorCount) * (100 / 25) : 35;

    console.log(
      `[CompetitiveDataProcessor] Calculated composite competitive score: ${finalScore.toFixed(2)} from ${factorCount} factors`,
    );
  ...
  private extractShapValues(record: any): Record<string, number> {
    if (
      (record as any).shap_values &&
      typeof (record as any).shap_values === "object"
    ) {
  ...
  private getTopContributingFields(record: any): Record<string, number> {
    ...
    fieldDefinitions.forEach((fieldDef) => {
      const sourceKey = Array.isArray(fieldDef.source)
        ? fieldDef.source[0]
        : fieldDef.source;
  ...
  private calculateCompetitiveStatistics(
    ...
    const median =
      total % 2 === 0
        ? (sorted[Math.floor(total / 2) - 1] + sorted[Math.floor(total / 2)]) /
          2
        : sorted[Math.floor(total / 2)];

    const variance =
      scores.reduce((acc, val) => acc + Math.pow(val - mean, 2), 0) / total;
    ...
    const avgMarketShare =
      normalizedShares.reduce((a, b) => a + b, 0) /
      (normalizedShares.length || 1);
    const marketConcentration =
      this.calculateMarketConcentration(normalizedShares);
    const competitiveIntensity = this.calculateCompetitiveIntensity(scores);

    console.log(
      "[CompetitiveDataProcessor] Competitive quintiles calculated:",
      competitiveQuintiles,
    );
    console.log(
      "[CompetitiveDataProcessor] Market share quintiles calculated:",
      marketShareQuintiles,
    );
  ...
  private calculateMarketConcentration(marketShares: number[]): number {
    ...
    const hhi = marketShares.reduce(
      (sum, share) => sum + Math.pow(share, 2),
      0,
    );
  ...
  private analyzeCompetitiveLandscape(records: GeographicDataPoint[]): any {
    ...
    const categoryAnalysis = Array.from(categoryMap.entries()).map(
      ([category, categoryRecords]) => {
        const avgScore =
          categoryRecords.reduce((sum, r) => sum + Number(r.value || 0), 0) /
          categoryRecords.length;
        const avgMarketShare =
          categoryRecords.reduce(
            (sum, r) =>
              sum + (Number((r.properties as any)?.market_share) || 0),
            0,
          ) / categoryRecords.length;
    ...
    const growthOpportunities = records
      .filter(
        (r) =>
          r.category === "challenged" &&
          (Number((r.properties as any)?.market_share) || 0) < 0.3,
      )
      .sort(
        (a, b) =>
          (Number((b.properties as any)?.brand_awareness) || 0) -
          (Number((a.properties as any)?.brand_awareness) || 0),
      )
      .slice(0, 5);
  ...
  private assessCompetitiveBalance(categoryAnalysis: any[]): string {
    const dominantPercentage =
      categoryAnalysis.find((c) => c.category === "dominant")?.percentage || 0;
    const competitivePercentage =
      categoryAnalysis.find((c) => c.category === "competitive")?.percentage ||
      0;
  ...
  private processCompetitiveFeatureImportance(
    rawFeatureImportance: any[],
  ): any[] {
    return rawFeatureImportance
      .map((it) => {
        const item = it as any;
        const featureName = String(
          (item as any).feature ?? (item as any).name ?? "unknown",
        );
        const importance = Number(
          (item as any).importance ?? (item as any).value ?? 0,
        );
        return {
          feature: featureName,
          importance,
  ...
  private generateCompetitiveSummary(
    ...
    const avgScore =
      records.reduce((sum, r) => sum + Number(r.value || 0), 0) / recordCount;
    ...
    const targetBrandName =
      firstProps?.target_brand_name || this.brandResolver.getTargetBrandName();
    const competitorBrandName =
      firstProps?.primary_competitor_name || "Primary Competitor";
...
`;
    ...
    const avgTargetShare =
      records.reduce((sum, r) => {
        const targetShare =
          Number((r.properties as any)?.target_brand_share) || 0;
        return sum + targetShare;
      }, 0) / recordCount;

    const avgCompetitorShare =
      records.reduce((sum, r) => {
        const competitorShare =
          Number((r.properties as any)?.primary_competitor_share) || 0;
        return sum + competitorShare;
      }, 0) / recordCount;

    const avgMarketGap = 100 - avgTargetShare - avgCompetitorShare;
    const avgWealthIndex =
      records.reduce(
        (sum, r) =>
          sum + (Number((r.properties as any)?.value_WLTHINDXCY) || 100),
        0,
      ) / recordCount;
    const avgIncome =
      records.reduce((sum, r) => {
        ...
        const income =
          Number((r.properties as any)?.value_AVGHINC_CY) || wealth * 500;
        return sum + income;
      }, 0) / recordCount;
    const avgPopulation =
      records.reduce(
        (sum, r) => sum + (Number((r.properties as any)?.value_TOTPOP_CY) || 0),
        0,
      ) / recordCount;
...
\n**🎯 Top Expansion Opportunities** (Markets with GROWTH potential, not current ${targetBrandName} dominance): `;
    ...
    const strongMarkets = records
      .filter((r) => r.value >= 60 && r.value < 80)
      .slice(0, 10);
    const emergingMarkets = records
      .filter((r) => r.value >= 40 && r.value < 60)
      .slice(0, 10);
...
\n**🏆 Premium Expansion Targets** (80+ expansion scores - high growth potential): `;
      topTierMarkets.forEach((record) => {
        const targetShare =
          Number(
            (((record as any).properties || {}) as any)?.target_brand_share,
          ) || 0;
...
\n**Strategic Expansion Insights:** `;
    ...
    if (competitiveAnalysisData) {
      const dominantMarkets = competitiveAnalysisData.find(
        (cat: any) => cat.category === "dominant",
      );
   ...
   */
  private calculateQuartileBreaks(
    values: number[],
  ...
  private formatClassLabel(
    classIndex: number,
    breaks: Array<{ min: number; max: number }>,
  ): string {
    if (classIndex === 0) {
