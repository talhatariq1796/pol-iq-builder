...
import {
  ...
  getTopFieldDefinitions,
} from "./HardcodedFieldDefs";
...
export class StrategicAnalysisProcessor extends BaseProcessor {
  ...
  validate(rawData: RawAnalysisResult): boolean {
    ...
    if (firstRecord?.type === "Feature" && firstRecord?.properties) {
      ...
      const hasBasicPropertyInfo =
        props.id || props.mls_number || props.address;
      const hasNumericFields = Object.keys(props).some(
        (key) => typeof props[key] === "number" && props[key] > 0,
      );
      console.log(
        `[StrategicProcessor] GeoJSON validation: hasBasicPropertyInfo=${hasBasicPropertyInfo}, hasNumericFields=${hasNumericFields}`,
      );
      return hasBasicPropertyInfo && hasNumericFields;
    }

    // For traditional format, check for strategic analysis fields
    const hasRequiredFields = (rawData.results as any[]).some(
      (record: any) =>
        record &&
        ...
        ((record as any).strategic_score !== undefined ||
          ...
          Object.keys(record).some(
            (key) =>
              key.includes("MP101") || key.toLowerCase().includes("share"),
          ) ||
    ...
    console.log(
      `[StrategicProcessor] Traditional format validation: ${hasRequiredFields}`,
    );
  ...
  process(
    ...
    console.log(
      `🎯 [STRATEGIC ANALYSIS PROCESSOR] Processing ${rawData.results?.length || 0} records for strategic expansion analysis`,
    );
    ...
    if (rawData.results && rawData.results.length > 0) {
      ...
      console.log(
        "🚨 [STRATEGIC PROCESSOR] First raw record DESCRIPTION:",
        first?.DESCRIPTION,
      );
      console.log(
        "🚨 [STRATEGIC PROCESSOR] First raw record keys:",
    ...
    if (!rawData.results || rawData.results.length === 0) {
      ...
      return {
        ...
        records: [],
        summary:
          getScoreExplanationForAnalysis(
            "strategic-analysis",
            "strategic_score",
          ) +
          "**Strategic Market Analysis Complete:** 0 geographic areas evaluated for expansion potential. ",
        featureImportance: this.processFeatureImportance(
          (rawData.feature_importance as any[]) || [],
        ),
    ...
    this.scoreField =
      getPrimaryScoreField(
        "strategic_analysis",
        (rawData as any)?.metadata ?? undefined,
      ) || "strategic_score";
    ...
    const viewMode =
      analysisOptions?.viewMode ?? context?.viewMode ?? "aggregate";
    ...
    if (
      rawRecords.length > 0 &&
      rawRecords[0]?.type === "Feature" &&
      rawRecords[0]?.properties
    ) {
      if (viewMode === "detail") {
        console.log(
          `🎯 [STRATEGIC PROCESSOR] Detected GeoJSON property data, returning detail listings view`,
        );
        return this.processPropertyListingsDetail(rawRecords, drilldownKey);
      }
      console.log(
        `🎯 [STRATEGIC PROCESSOR] Detected GeoJSON property data, creating market aggregations`,
      );
      return this.processPropertyListingsAsMarketData(
        rawRecords,
        drilldownKey,
        viewMode,
      );
    ...
    if (
      rawRecords.length > 0 &&
      rawRecords[0].strategic_analysis_score !== undefined &&
      rawRecords[0].strategic_score === undefined
    ) {
      this.scoreField = "strategic_analysis_score";
      console.log(
        `🎯 [STRATEGIC PROCESSOR] Detected strategic_analysis_score field in test data, using as primary field`,
      );
    }
    const processedRecords = rawRecords.map((record: any, index: number) => {
      ...
      console.log(
        `[StrategicAnalysisProcessor] Record ${recordId}: Found primaryScore = ${primaryScore} from field: ${this.scoreField}=${(record as any)[this.scoreField]}`,
      );

      // Debug brand fields and market gap calculation
      if (recordId === (rawRecords as any[])[0]?.ID) {
        console.log(
          `[StrategicAnalysisProcessor] DEBUG First Record Brand Detection:`,
          {
      ...
      const looksLikeShare =
        /share|rate|pct|percent/i.test(this.scoreField || "") ||
        (record as any).strategic_score === undefined;
      ...
      if (primaryScore !== null && !isNaN(primaryScore)) {
        if (looksLikeShare && primaryScore < 20 && !isStrategicScoreField) {
          console.warn(
            `[StrategicAnalysisProcessor] Record ${recordId}: Score ${primaryScore} from "${this.scoreField}" appears to be share/rate data, using composite score`,
          );
        ...
        console.warn(
          `[StrategicAnalysisProcessor] Record ${recordId}: No strategic score found, calculating composite score`,
        );
        primaryScore = this.calculateCompositeStrategicScore(record);
      }
      ...
      if (isNaN(primaryScore) || primaryScore < 0) {
        console.warn(
          `[StrategicAnalysisProcessor] Record ${recordId}: Invalid strategic score, using default of 25`,
        );
      ...
      const { strategic_score: _, ...filteredTopFields } =
        topContributingFields as any;
      ...
      const processed: any = {
        ...
        ...(record.strategic_analysis_score !== undefined
          ? { strategic_analysis_score: Math.round(primaryScore * 100) / 100 }
          : {}),
        ...
        total_population: this.extractNumericValue(
          record,
          this.configManager.getFieldMapping("populationField"),
          0,
        ),
        median_income: this.extractNumericValue(
          record,
          this.configManager.getFieldMapping("incomeField"),
          0,
        ),
        competitive_advantage_score: this.extractFieldValue(record, [
          "competitive_advantage_score",
          "comp_advantage",
          "advantage_score",
        ]),
        diversity_index: this.extractFieldValue(record, [
          "diversity_index",
          "value_DIVINDX_CY",
          "DIVINDX_CY",
        ]),
        properties: {
          ...
          market_gap: this.calculateRealMarketGap(record),
          total_population: this.extractFieldValue(record, [
            "total_population",
            "value_TOTPOP_CY",
            "TOTPOP_CY",
            "population",
          ]),
          median_income: this.extractFieldValue(record, [
            "median_income",
            "value_AVGHINC_CY",
            "AVGHINC_CY",
            "household_income",
          ...
          demographic_opportunity_score: this.extractFieldValue(record, [
    ...
    */
    ...
    console.log(
      `🎯 [STRATEGIC PROCESSOR] Filtered ${rankedRecords.length - nonParkRecords.length} parks from analysis`,
    );
    console.log(`🎯 [STRATEGIC PROCESSOR] Top 5 non-park strategic markets:`);
    nonParkRecords.slice(0, 5).forEach((record, i) => {
      console.log(
        `🎯   ${i + 1}. ${(record as any).area_name} (${record.area_id}): ${(record as any).value}`,
      );
    });

    // Extract feature importance
    const featureImportance = this.processFeatureImportance(
      (rawData.feature_importance as any[]) || [],
    );
  ...
  private processPropertyListingsDetail(
    geoJsonRecords: any[],
    drilldownKey?: string,
  ): ProcessedAnalysisData {
    console.log(
      `🏠 [STRATEGIC PROCESSOR] Rendering property detail view (${geoJsonRecords.length} listings, drilldownKey=${drilldownKey})`,
    );

    const normalizeMarketKey = (props: any) => {
      if (!props) return undefined;
      const rawMunicipality =
        props.municipality || props.city || props.community;
      const rawPostal =
        props.postal_code || props.postalCode || props.zip || props.fsa_code;
    ...
    const candidateRecords = normalizedKey
      ? geoJsonRecords.filter((feature) => {
          ...
          const municipality = (props.municipality ||
            ...
            props.community ||
            "") as string;
          const postal = (props.postal_code ||
            props.postalCode ||
            props.zip ||
            props.fsa_code ||
            "") as string;
          return [municipality, postal.substring(0, 3)].some(
            (candidate) =>
              candidate &&
              String(candidate).toUpperCase().includes(normalizedKey),
          );
        })
      : geoJsonRecords;

    const limitedRecords = candidateRecords.slice(
      0,
      Math.min(candidateRecords.length, 250),
    );
    if (candidateRecords.length > limitedRecords.length) {
      console.log(
        `🏠 [STRATEGIC PROCESSOR] Detail view truncated to ${limitedRecords.length} listings (of ${candidateRecords.length})`,
      );
    }
    ...
    const toListingScore = (props: Record<string, any>) => {
      ...
      for (const value of candidates) {
        ...
        if (Number.isFinite(numeric) && numeric !== 0) {
          ...
          console.log(
            `🏠 [toListingScore] Found candidate score: ${numeric} -> capped to ${cappedScore}`,
          );
      ...
      const askedPrice = Number(
        props?.asked_price ?? props?.price ?? props?.list_price ?? 0,
      );
      const soldRatio = Number(
        props?.sold_ratio ?? props?.liquidity_score ?? 0,
      );
      ...
      const liquidityScore =
        soldRatio > 0 ? Math.min((soldRatio / 100) * 30, 30) : 0; // 0-30 points
      const priceScore =
        askedPrice > 0 ? Math.min((askedPrice / 3000000) * 30, 30) : 0; // 0-30 points (3M+ = max)

      const heuristicScore = Math.min(
        100,
        bedroomScore + liquidityScore + priceScore,
      );
      console.log(
        `🏠 [toListingScore] Using heuristic: bedrooms=${bedroomScore.toFixed(1)}, liquidity=${liquidityScore.toFixed(1)}, price=${priceScore.toFixed(1)}, total=${heuristicScore.toFixed(1)}`,
      );
      return Math.round(Math.max(heuristicScore, 25));
    };

    const detailRecords: GeographicDataPoint[] = limitedRecords.map(
      (feature, index) => {
        const props = feature?.properties ?? {};
        const listingId =
          props.id ||
        ...
        return {
          ...
          properties: {
            ...
            formatted_price: props?.asked_price?.toLocaleString?.("en-US", {
              style: "currency",
              currency: "USD",
            }),
  ...
  private extractCoordinatesFromListing(
    feature: any,
  ...
  private createStrategicRenderer(records: GeographicDataPoint[]): any {
    console.log(
      `🎯 [STRATEGIC RENDERER] Creating direct renderer for ${records.length} records`,
    );
    ...
    const sampleGeometryType =
      sampleGeometryRecord && (sampleGeometryRecord.geometry as any)?.type;
    const isPointGeometry =
      typeof sampleGeometryType === "string" &&
      sampleGeometryType.toLowerCase() === "point";
    ...
    for (let i = 0; i < quartileBreaks.length - 1; i++) {
      console.log(
        `🎯 [STRATEGIC RENDERER] Class ${i + 1}: ${quartileBreaks[i]} - ${quartileBreaks[i + 1]} -> [${strategicColors[i].join(", ")}]`,
      );
    ...
    console.log(`🎯 [STRATEGIC RENDERER] Created renderer:`, {
      ...
      sampleRecordStrategicScore:
        (records[0] as any)?.[this.scoreField] ||
        (records[0]?.properties as any)?.[this.scoreField],
    });
   ...
   */
  private formatClassLabel(
    classIndex: number,
    quartileBreaks: number[],
  ): string {
  ...
  private getTopContributingFields(record: any): Record<string, number> {
    const contributingFields: Array<{
      field: string;
      value: number;
      importance: number;
    }> = [];
    ...
    fieldDefinitions.forEach((fieldDef) => {
      let value = 0;
      const sources = Array.isArray(fieldDef.source)
        ? fieldDef.source
        : [fieldDef.source];
    ...
    console.log(
      `[StrategicAnalysisProcessor] Top contributing fields for ${(record as any).ID}:`,
      topFields,
    );
    return topFields;
  }
  ...
  private calculateStatisticsFromRecords(
  ...
  private generateStrategicSummary(
    ...
    let summary = getScoreExplanationForAnalysis(
      "strategic-analysis",
      "strategic_score",
    );
    ...
    console.log(
      `🎯 [STRATEGIC SUMMARY] Top 10 strategic markets:`,
      topMarkets.slice(0, 5).map((r) => `${r.area_name} (${r.value})`),
    );
    if (topMarkets.length > 0) {
      summary += `**Top Strategic Markets:** `;
      const topNames = topMarkets.map(
        (r) => `${r.area_name} (${r.value.toFixed(1)})`,
      );
      summary += `${topNames.join(", ")}. `;

      // Analyze characteristics of top markets
      const avgMarketGap =
        topMarkets.reduce((sum, r) => {
    ...
    const highPotential = records.filter(
      (r) => r.value >= (statistics.percentile75 || statistics.mean),
    ).length;
    if (records.length > 0) {
    ...
    const untappedMarkets = records.filter(
      (r) => Number((r.properties as any)?.market_gap) > 80,
    ).length;
  ...
  private extractTargetBrandShare(record: any): number {
    ...
    if (record.ID === (record as any).ID && brandFields.length > 0) {
      console.log(
        `[StrategicAnalysisProcessor] Brand detection for ${record.ID}:`,
        {
          brandFieldsCount: brandFields.length,
          targetBrand: targetBrand
            ? { name: targetBrand.metricName, value: targetBrand.value }
            : "none",
          allBrands: brandFields.map((bf) => ({
            name: bf.metricName,
            value: bf.value,
            isTarget: bf.isTarget,
          })),
        },
      );
    }
  ...
  private calculateCompositeStrategicScore(record: any): number {
    ...
    const population = this.extractFieldValue(record, [
      "total_population",
      "value_TOTPOP_CY",
      "TOTPOP_CY",
    ]);
    ...
    const income = this.extractFieldValue(record, [
      "median_income",
      "value_AVGHINC_CY",
      "AVGHINC_CY",
    ]);
    ...
    const competitiveScore = this.extractFieldValue(record, [
      "competitive_advantage_score",
      "comp_advantage",
    ]);
    if (competitiveScore > 0) {
      // If on 1-10 scale, convert to 0-100
      const normalizedCompetitive =
        competitiveScore <= 10 ? competitiveScore * 10 : competitiveScore;
    ...
    console.log(
      `[StrategicAnalysisProcessor] Calculated composite strategic score: ${finalScore.toFixed(2)} from ${factorCount} factors for record ${record.ID || "unknown"}`,
    );
  ...
  private processPropertyListingsAsMarketData(
    ...
    console.log(
      `🏠 [STRATEGIC PROCESSOR] Processing ${geoJsonRecords.length} property listings as market data`,
    );
    ...
    console.log(
      `🏠 [STRATEGIC PROCESSOR] Created ${marketGroups.size} market groups from property data`,
    );

    // Create market analysis records
    const records: GeographicDataPoint[] = Array.from(
      marketGroups.entries(),
    ).map(([marketId, properties], index) => {
      const propertyCount = properties.length;

      const collectNumeric = (
        accessor: (props: any) => number | string | null | undefined,
      ) => {
        return properties
      ...
      const population = collectNumeric(
        (props) => props?.total_population ?? props?.population,
      );
      const income = collectNumeric(
        (props) => props?.median_income ?? props?.avg_household_income,
      );
      const homeownership = collectNumeric(
        (props) => props?.homeownership_rate,
      );
      const populationDensity = collectNumeric(
        (props) => props?.population_density,
      );
      ...
      const soldCount = properties.filter(
        (p) => Number(p.properties?.is_sold) === 1,
      ).length;
      const soldRatio =
        propertyCount > 0 ? (soldCount / propertyCount) * 100 : 0;

      const avgHomeownershipRateRaw = averageOf(homeownership);
      const avgHomeownershipRate =
        avgHomeownershipRateRaw <= 1
          ? avgHomeownershipRateRaw * 100
          : avgHomeownershipRateRaw;

      const avgRentalYieldRaw = averageOf(rentalYield);
      const avgRentalYield =
        avgRentalYieldRaw <= 1 ? avgRentalYieldRaw * 100 : avgRentalYieldRaw;

      const fsaCodes = Array.from(
        new Set(
          properties
            .map(
              (p) =>
                p.properties?.fsa_code ||
                (p.properties?.postal_code
                  ? String(p.properties.postal_code).substring(0, 3)
                  : null),
            )
            .filter(Boolean)
      ...
      const coordinateCandidates: Array<[number, number]> = properties
        ...
        .filter(
          (coord) =>
            Array.isArray(coord) &&
            coord.length === 2 &&
            coord.every((value) => Number.isFinite(Number(value))),
        )
      ...
      const centroid =
        coordinateCandidates.length > 0
          ? [
              coordinateCandidates.reduce((sum, coord) => sum + coord[0], 0) /
                coordinateCandidates.length,
              coordinateCandidates.reduce((sum, coord) => sum + coord[1], 0) /
                coordinateCandidates.length,
            ]
          : [
              Number(firstProp.geometry?.coordinates?.[0]) ||
                Number(firstProp.properties?.longitude) ||
                0,
              Number(firstProp.geometry?.coordinates?.[1]) ||
                Number(firstProp.properties?.latitude) ||
                0,
            ];

      const hasValidCentroid = centroid.every((value) =>
        Number.isFinite(value),
      );
      const coordinates: [number, number] | undefined = hasValidCentroid
      ...
      const affordabilityScore =
        avgPrice > 0 && medianPrice > 0
          ? Math.min(100, (medianPrice / Math.max(avgPrice, 1)) * 100)
          : 50;
      const demographicScore = Math.min(
        100,
        (averageOf(population) > 0
          ? Math.min((averageOf(population) / 500000) * 100, 100)
          : 0) *
          0.4 +
          (averageOf(income) > 0
            ? Math.min((averageOf(income) / 120000) * 100, 100)
            : 0) *
            0.6,
      );
      ...
      const primaryAreaId =
        fsaCodes[0] || String(marketId || `market_${index}`);
      ...
      const aggregatedRecord: GeographicDataPoint = {
        ...
        area_name:
          typeof marketId === "string"
            ? marketId
            : String(marketId ?? "Unknown Market"),
    ...
    const percentile = (p: number) => {
      ...
      const clampedIndex = Math.max(
        0,
        Math.min(sortedValues.length - 1, index),
      );
    ...
    const topProperties = topRecord?.properties as
      | Record<string, any>
      | undefined;
    ...
    const topMarketActiveListings =
      typeof topProperties?.property_count === "number"
        ? topProperties.property_count
        : undefined;
    const topMarketSoldRatio =
      typeof topProperties?.sold_ratio === "number"
    ...
    return {
      ...
      featureImportance: [
        {
          feature: "property_count",
          importance: 0.4,
          description: "Number of active listings indicating market activity",
        },
        {
          feature: "avg_bedrooms",
          importance: 0.3,
          description: "Average bedrooms indicating property quality",
        },
        {
          feature: "avg_bathrooms",
          importance: 0.3,
          description: "Average bathrooms indicating property amenities",
        },
      ],
      statistics,
      targetVariable: "strategic_score",
