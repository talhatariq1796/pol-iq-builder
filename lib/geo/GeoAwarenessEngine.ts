...
export class GeoAwarenessEngine {
  ...
  public async processGeoQuery(
    ...
    try {
      ...
      return {
        ...
        warnings: [
          `Geographic filtering failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        ],
      };
    }
  }
  ...
  private async parseGeographicQuery(query: string): Promise<GeoQuery> {
    ...
    if (!hasExplicitGeoIntent) {
      console.log(
        `🚫 [GEO DEBUG] Query lacks explicit geographic intent - skipping geographic filtering`,
      );
      return {
        originalQuery: query,
        entities: [],
    ...
    console.log(
      `✅ [GEO DEBUG] Query has explicit geographic intent - proceeding with matching`,
    );

    // Method 1: Direct entity matching with hierarchy (most reliable)
    const directMatches = this.findDirectMatches(queryLower);
    console.log(
      `🔍 [GEO DEBUG] Direct matches:`,
      directMatches.map((e) => ({ name: e.name, type: e.type })),
    );
    console.log(
      `🔍 [GEO DEBUG] Direct match details:`,
      directMatches.map((e) => ({
        name: e.name,
        type: e.type,
    ...
    console.log(
      `🔍 [GEO DEBUG] ZIP matches:`,
      zipMatches.map((e) => ({ name: e.name, type: e.type })),
    );
    ...
    console.log(
      `🔍 [GEO DEBUG] Regional matches:`,
      regionalMatches.map((e) => ({ name: e.name, type: e.type })),
    );
    ...
    if (entities.length === 0 && this.hasStrongGeographicSignals(queryLower)) {
      ...
      console.log(
        `🔍 [GEO DEBUG] Fuzzy matches (with strong signals):`,
        fuzzyMatches.map((e) => ({ name: e.name, type: e.type })),
      );
      entities.push(...fuzzyMatches);
    } else if (entities.length === 0) {
      console.log(
        `🚫 [GEO DEBUG] No geographic entities found and no strong geographic signals - treating as non-geographic query`,
      );
    }
  ...
  private async applyGeographicFilter(
    ...
    const geographicallyEligibleRecords =
      this.filterOutNonGeographicLocations(records);

    console.log(
      `[GeoAwarenessEngine] Pre-filtered records: ${records.length} -> ${geographicallyEligibleRecords.length} (removed street names/businesses)`,
    );
    ...
    const strategies = [
      ...
      { method: "hierarchical", fn: this.filterByHierarchy.bind(this) },
      {
        ...
        fn: this.filterByDescriptionPattern.bind(this),
      },
    ...
    for (const strategy of strategies) {
      try {
        ...
        console.warn(
          `[GeoAwarenessEngine] Strategy ${strategy.method} failed:`,
          error,
        );
        fallbackUsed = true;
      }
    ...
    if (!bestResult || bestResult.filteredRecords.length === 0) {
      bestResult = {
        ...
        fallbackUsed: true,
        warnings: [
          "No geographic matches found, showing pre-filtered data (street names excluded)",
        ],
      };
    }
  ...
  private async filterByZipCode(
    ...
    for (const entity of geoQuery.entities) {
      ...
      if (
        ...
        console.log(
          `[GeoAwarenessEngine] Adding ${entity.zipCodes?.length || 0} ZIP codes for city: ${entity.name}`,
        );
      }
      // Only expand ZIP codes for broader entities if no specific cities were mentioned
      else if (
        geoQuery.entities.every(
          (e) => e.type !== "city" && e.type !== "borough",
        )
      ) {
    ...
    console.log(
      `[GeoAwarenessEngine] Phase 1 ZIP filtering: Found ${targetZipCodes.size} target ZIP codes for entities:`,
  ...
  private async filterByFieldScanning(
    ...
    for (const entity of geoQuery.entities) {
      const searchTerms = [entity.name, ...entity.aliases].map((term) =>
        term.toLowerCase(),
      );
  ...
  private initializeGeographicData(): void {
    try {
      ...
      console.error(
        "[GeoAwarenessEngine] Failed to initialize geographic data:",
        error,
      );
  ...
  private hasExplicitGeographicIntent(query: string): boolean {
    ...
    const hasKnownGeographicEntity =
      this.queryContainsGeographicEntity(queryLower);
    ...
    const hasBusinessContext = businessContextPatterns.some((pattern) =>
      pattern.test(query),
    );
    ...
    const hasGeographicPatterns = allPatterns.some((pattern) =>
      pattern.test(query),
    );
    ...
    if (hasBusinessContext) {
      console.log(
        `🚫 [GEO DEBUG] Business context detected - requiring explicit geographic indicators`,
      );
      ...
      const hasExplicitGeo = explicitGeoPatterns.some((pattern) =>
        pattern.test(query),
      );
      console.log(
        `🚫 [GEO DEBUG] Explicit geographic patterns found: ${hasExplicitGeo}`,
      );
      return hasExplicitGeo;
    }

    console.log(
      `✅ [GEO DEBUG] No business context - using standard geographic detection`,
    );
    console.log(
      `✅ [GEO DEBUG] Geographic patterns: ${hasGeographicPatterns}, Known entities: ${hasKnownGeographicEntity}`,
    );
  ...
  private findDirectMatches(query: string): GeographicEntity[] {
    ...
    const uniqueMatches = this.deduplicateAndPrioritizeMatches(allMatches);
   ...
   */
  private deduplicateAndPrioritizeMatches(
    matches: GeographicEntity[],
  ): GeographicEntity[] {
    ...
    const uniqueMatches = matches.filter(
      (match, index, self) =>
        index ===
        self.findIndex((m) => m.name === match.name && m.type === match.type),
    );
    ...
    if (
      cityMatches.some((m) => m.name.toLowerCase().includes("quebec")) &&
      stateMatches.some((m) => m.name.toLowerCase() === "quebec")
    ) {
      return uniqueMatches.filter(
        (m) => !(m.type === "state" && m.name.toLowerCase() === "quebec"),
      );
    }
  ...
  private findRegionalMatches(query: string): GeographicEntity[] {
    ...
    const regionalPatterns = [
      {
        pattern: /\b(northeast|northeastern|north east)\b/,
        region: "northeastern united states",
      },
      {
        pattern: /\b(southeast|southeastern|south east)\b/,
        region: "southeastern united states",
      },
      {
        pattern: /\b(midwest|midwestern|middle west)\b/,
        region: "midwestern united states",
      },
      {
        pattern: /\b(southwest|southwestern|south west)\b/,
      ...
      {
        pattern: /\b(los angeles metro|la metro|greater los angeles)\b/,
  ...
  private determineSpatialRelation(
    query: string,
  ): "within" | "near" | "contains" | "intersects" | undefined {
    const lowerQuery = query.toLowerCase();
  ...
  private deduplicateEntities(
    ...
    return filtered.filter((entity) => {
      ...
      if (entity.type === "county" || entity.type === "state") {
        const entityNameBase = entity.name
          .toLowerCase()
          .replace(/[éèêë]/g, "e")
          .replace(/[àâä]/g, "a");
        for (const cityName of cityNames) {
          const cityNameBase = cityName
            .replace(/[éèêë]/g, "e")
            .replace(/[àâä]/g, "a");
          // If the city and region names are similar (e.g., "Montreal" and "Montréal"), keep only the city
          if (
            entityNameBase === cityNameBase ||
            entityNameBase.includes(cityNameBase) ||
            cityNameBase.includes(entityNameBase)
          ) {
            console.log(
              `🔍 [GEO DEBUG] Excluding region "${entity.name}" because city "${cityName}" is already matched`,
            );
            return false;
          }
        }
  ...
  private expandEntityHierarchy(entity: GeographicEntity): GeographicEntity[] {
    ...
    if (entity.childEntities) {
      for (const childName of entity.childEntities) {
        const childEntity = this.geographicHierarchy.get(
  ...
  private isValidLocationMatch(
    ...
    const rejectPatterns = streetSuffixes.map(
      (suffix) => new RegExp(`\\b${locationLower}\\s+${suffix}\\b`, "i"),
    );

    // Also reject if it's the same city name in a different state or business name
    const otherStateAbbrevs = [
      "al",
      "ak",
      "az",
      "ar",
    ...
    const invalidStatePatterns = otherStateAbbrevs
      ...
      .map(
        (state) => new RegExp(`\\b${locationLower}\\s*,\\s*(${state})\\b`, "i"),
      );

    // Reject business/restaurant names that just happen to include city names
    const businessPatterns = [
      new RegExp(
        `\\b${locationLower}\\s+(restaurant|diner|cafe|bar|grill|wild wings|pizza|burger|wings)\\b`,
        "i",
      ),
      new RegExp(
        `\\b${locationLower}\\s+(hotel|motel|inn|lodge|resort)\\b`,
        "i",
      ),
      new RegExp(
        `\\b${locationLower}\\s+(mall|shopping|outlet|center|plaza)\\b`,
        "i",
      ),
    ];
    ...
    if (
      rejectPatterns.some((pattern) => pattern.test(textLower)) ||
    ...
    const validStatePatterns = validStates.map(
      (state) => new RegExp(`\\b${locationLower}\\s*,\\s*${state}\\b`, "i"),
    );
    ...
    if (entity.type === "city" || entity.type === "borough") {
      const neighborhoodContext = this.hasNeighborhoodContext(
        textLower,
        locationLower,
      );
  ...
  private createDescriptionPatterns(entity: GeographicEntity): RegExp[] {
    ...
    if (entity.type === "city" || entity.type === "borough") {
      ...
      if (stateInfo.abbreviations.length > 0) {
        ...
        patterns.push(
          new RegExp(`\\b${entityName}\\s*,\\s*(${abbrevPattern})\\b`, "i"),
        );
      }
      ...
      if (stateInfo.fullNames.length > 0) {
        ...
        patterns.push(
          new RegExp(`\\b${entityName}\\s*,\\s*(${fullNamePattern})\\b`, "i"),
        );
      }
      ...
      patterns.push(
        new RegExp(
          `\\b${entityName}(?!\\s+(Avenue|Street|Road|Boulevard|Drive|Lane|Way|Court|Place))\\b`,
          "i",
        ),
      );
      patterns.push(
        new RegExp(
          `,\\s*${entityName}(?!\\s+(Avenue|Street|Road|Boulevard|Drive|Lane|Way|Court|Place))`,
          "i",
        ),
      );
      ...
      patterns.push(
        new RegExp(
          `\\b${entityName}\\s+(area|region|metro|metropolitan)\\b`,
          "i",
        ),
      );
    ...
    if (patterns.length === 0) {
      ...
      if (
        ...
        patterns.push(
          new RegExp(`\\b${this.escapeRegex(entity.name)}\\b`, "i"),
        );
        ...
        patterns.push(
          new RegExp(
            `\\b${this.escapeRegex(entity.name)}\\s*,\\s*[A-Z]{2}\\b`,
            "i",
          ),
        ); // "City, ST"
        patterns.push(
          new RegExp(`\\(${this.escapeRegex(entity.name)}\\)`, "i"),
        ); // "(City)"
      }
    }
   ...
   */
  private getStateInfo(entity: GeographicEntity): {
    ...
    if (!state) {
      ...
      if (entity.zipCodes && entity.zipCodes.length > 0) {
        ...
        if (
          firstZip.startsWith("10") ||
          firstZip.startsWith("11") ||
          firstZip.startsWith("12") ||
          firstZip.startsWith("13") ||
          firstZip.startsWith("14")
        ) {
          state = "ny";
        } else if (firstZip.startsWith("07") || firstZip.startsWith("08")) {
          state = "nj";
        } else if (
          firstZip.startsWith("15") ||
          firstZip.startsWith("16") ||
          firstZip.startsWith("17") ||
          firstZip.startsWith("18") ||
          firstZip.startsWith("19")
        ) {
          state = "pa";
        }
      }
    ...
    for (const term of searchTerms) {
      ...
      for (const word of words) {
        if (word.length >= 3 && term.length >= 3) {
          if (
            this.editDistance(word, term) <=
            Math.min(2, Math.floor(term.length / 3))
          ) {
            return true;
          }
        }
