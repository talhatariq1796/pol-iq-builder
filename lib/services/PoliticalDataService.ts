...
import {
  ...
  parsePaCountyFpFromPrecinctKey,
...
// Blob URL keys — sourced from the active state config
const BLOB_KEYS = {
  precinctBoundaries: activeState.blobKeys.precinctBoundaries,
  electionResults: activeState.blobKeys.electionResults,
  targetingScores: activeState.blobKeys.targetingScores,
  politicalScores: activeState.blobKeys.politicalScores,
  demographics: activeState.blobKeys.demographics,
  crosswalk: activeState.blobKeys.crosswalk,
  districtCrosswalk: activeState.blobKeys.districtCrosswalk,
  h3Aggregates: activeState.blobKeys.h3Aggregates,
  h3GeoJSON: activeState.blobKeys.h3GeoJSON,
  paH3Aggregates: activeState.blobKeys.stateH3Aggregates,
  paH3GeoJSON: activeState.blobKeys.stateH3GeoJSON,
  modelRegistry: activeState.blobKeys.modelRegistry,
};

// Fallback local paths (for development) — sourced from the active state config
const LOCAL_PATHS = {
  precinctBoundaries: activeState.paths.precinctBoundaries,
  electionResults: activeState.paths.electionResults,
  targetingScores: activeState.paths.targetingScores,
  districtCrosswalk: activeState.paths.districtCrosswalk,
  politicalScores: activeState.paths.politicalScores,
  demographics: activeState.paths.demographics,
  paPrecinctDemographics: activeState.paths.precinctDemographics,
  crosswalk: activeState.paths.crosswalk,
  h3Aggregates: activeState.paths.h3Aggregates,
  h3GeoJSON: activeState.paths.h3GeoJSON,
  paH3Aggregates: activeState.paths.stateH3Aggregates,
  paH3GeoJSON: activeState.paths.stateH3GeoJSON,
};
...
async function loadBlobUrlMappings(): Promise<Record<string, string>> {
  ...
  try {
    ...
    let merged: Record<string, string> = {
      ...(blobUrlsBundled as Record<string, string>),
    };
    try {
    ...
    console.warn(
      "[PoliticalDataService] Failed to load blob URL mappings:",
      error,
    );
    blobUrlMappings = { ...(blobUrlsBundled as Record<string, string>) };
    return blobUrlMappings;
...
async function readJsonFromPublicPath(localPath: string): Promise<unknown> {
  ...
  if (origin) {
    ...
    try {
      ...
      console.warn(
        `[PoliticalDataService] readJsonFromPublicPath: fetch ${url} failed, trying fs`,
        e,
      );
    }
  }
...
async function nodePublicFetch(input: RequestInfo | URL): Promise<Response> {
  ...
  try {
    ...
    if (!origin) {
      throw new Error(
        `nodePublicFetch: missing file ${filePath} and no deployment origin (VERCEL_URL / NEXT_PUBLIC_SITE_URL)`,
      );
    }
 ...
 */
async function fetchFromBlobOrLocal<T>(
  blobKey: string,
  localPath: string,
): Promise<T> {
  ...
  if (blobUrl) {
    try {
      ...
      if (response.ok) {
        console.log(
          `[PoliticalDataService] Loaded ${blobKey} from blob storage`,
        );
        return await response.json();
      }
    } catch (error) {
      console.warn(
        `[PoliticalDataService] Blob fetch failed for ${blobKey}:`,
        error,
      );
    }
  }
  ...
  if (isNodeRuntime()) {
    ...
    try {
      ...
      console.error(
        `[PoliticalDataService] Failed to read ${blobKey} from public/:`,
        error,
      );
      throw error;
    }
  ...
  console.log(
    `[PoliticalDataService] Loading ${blobKey} from local path: ${localPath}`,
  );
  const response = await fetch(localPath);
  if (!response.ok) {
    throw new Error(
      `Failed to load ${blobKey} from ${localPath}: ${response.status}`,
    );
  }
  return await response.json();
}
...
export class PoliticalDataService {
   ...
   */
  private unifiedPrecinctDataPromise: Promise<
    Record<string, UnifiedPrecinct>
  > | null = null;
  ...
  private municipalityDataFileFormatPromise: Promise<MunicipalityDataFile> | null =
    null;
  /** Keys: chamber:state_house | county | school_districts | zip_codes */
  private paCompareAggregatedDistrictPromises: Map<
    string,
  ...
  async initialize(): Promise<void> {
    ...
    if (
      ...
      console.log(
        `[PoliticalDataService] Deployment version changed (${cachedDeploymentVersion} -> ${CURRENT_DEPLOYMENT_VERSION}), clearing cache...`,
      );
      this.clearCache();
    }
  ...
  private async _doInitialize(): Promise<void> {
    console.log(
      "[PoliticalDataService] Initializing from Vercel Blob Storage...",
    );
    ...
    try {
      ...
      console.log(
        `[PoliticalDataService] Loaded ${Object.keys(targetingScores.precincts).length} precinct targeting scores`,
      );
      console.log(
        `[PoliticalDataService] Loaded ${Object.keys(politicalScores.precincts).length} precinct political scores`,
      );
      console.log(
        `[PoliticalDataService] Loaded ${crosswalk.length} crosswalk entries`,
      );
      console.log(
        `[PoliticalDataService] Loaded ${Object.keys(elections.precincts || elections.precinctHistory || {}).length} precinct election records`,
      );
      console.log(
        `[PoliticalDataService] Loaded ${Object.keys(demographics.precincts).length} precinct demographics`,
      );

      // Load H3 aggregates (for heatmaps)
      const h3 = await this.loadH3Aggregates();
      console.log(
        `[PoliticalDataService] Loaded ${Object.keys(h3.cells).length} H3 cells`,
      );
    ...
    try {
      ...
      for (const feature of geojson.features) {
        ...
        if (feature.geometry?.type === "Polygon") {
          ...
          const multiCoords = (feature.geometry as GeoJSON.MultiPolygon)
            .coordinates;
      ...
      console.warn(
        "[PoliticalDataService] Failed to load precinct GeoJSON for centroids:",
        error,
      );
    }
  ...
  private async loadBAData(): Promise<void> {
    if (!BA_FEATURE_SERVICE_URL) {
      console.log(
        "[PoliticalDataService] No BA feature service URL configured",
      );
      return;
    }

    console.log(
      "[PoliticalDataService] Loading BA data from feature service...",
    );
    ...
    try {
      ...
      console.log(
        `[PoliticalDataService] Loaded BA data for ${cache.baData.size} block groups`,
      );
    } catch (error) {
   ...
   */
  async getPrecinctTargetingScores(
    precinctName: string,
  ): Promise<TargetingScoresPrecinct | null> {
   ...
   */
  async getAllTargetingScores(): Promise<
    Record<string, TargetingScoresPrecinct>
  > {
    await this.initialize();
   ...
   */
  async getTargetingScoresSummary(): Promise<
    TargetingScoresData["summary"] | null
  > {
    await this.initialize();
  ...
  async getPrecinctsByStrategy(strategy: string): Promise<string[]> {
    ...
    for (const [name, scores] of Object.entries(
      cache.targetingScores?.precincts || {},
    )) {
   ...
   */
  async getPrecinctScores(
    precinctName: string,
  ): Promise<PrecinctPoliticalScores | null> {
    await this.initialize();
  ...
  async getAllPrecinctScores(): Promise<Map<string, PrecinctPoliticalScores>> {
    ...
    for (const [name, raw] of Object.entries(
      cache.politicalScores?.precincts || {},
    )) {
  ...
  private async buildUnifiedPrecinctData(): Promise<
    Record<string, UnifiedPrecinct>
  > {
    ...
    for (const name of allPrecinctNames) {
      ...
      unified[name] = {
        ...
        demographics: {
          ...
          medianHHI:
            paRow?.median_household_income ??
            targetingData?.median_household_income ??
            0,
          collegePct: paRow?.college_pct ?? targetingData?.college_pct ?? 0,
          homeownerPct:
            paRow?.owner_pct ?? targetingData?.owner_pct ?? undefined,
          diversityIndex:
            paRow?.diversity_index ?? targetingData?.diversity_index ?? 0,
          populationDensity:
            paRow?.population_density ??
            targetingData?.population_density ??
            undefined,
        },
        ...
        political: {
          demAffiliationPct:
            paOutlook?.demAffiliationPct ??
            targetingData?.dem_affiliation_pct ??
            0,
          repAffiliationPct:
            paOutlook?.repAffiliationPct ??
            targetingData?.rep_affiliation_pct ??
            0,
          independentPct:
            paOutlook?.independentPct ??
            targetingData?.ind_affiliation_pct ??
            0,
          liberalPct: paOutlook?.liberalPct ?? targetingData?.liberal_pct ?? 0,
          moderatePct:
            paOutlook?.moderatePct ?? targetingData?.moderate_pct ?? 0,
          conservativePct:
        ...
        gotvComponents: targetingData?.gotv_components
          ? {
              ...
              turnoutOpportunity:
                targetingData.gotv_components.turnout_opportunity,
        ...
        persuasionComponents: targetingData?.persuasion_components
          ? {
              marginCloseness:
                targetingData.persuasion_components.margin_closeness,
              swingFactor: targetingData.persuasion_components.swing_factor,
              moderateFactor:
                targetingData.persuasion_components.moderate_factor,
              independentFactor:
                targetingData.persuasion_components.independent_factor,
   ...
   */
  async getUnifiedPrecinct(
    precinctName: string,
  ): Promise<UnifiedPrecinct | null> {
    ...
    for (const [key, value] of Object.entries(allData)) {
      const targetingData = targeting[key];
      const pn = targetingData?.precinct_name
        ?.toLowerCase()
        ?.replace(/\s+/g, " ");
      if (pn && pn === compact) {
        return value;
    ...
    for (const [key, value] of Object.entries(allData)) {
      ...
      if (
        shortName &&
        (shortName.includes(lowerName) || lowerName.includes(shortName))
      ) {
        return value;
      }
    }
  ...
  async resolvePrecinctMapKey(rawLabel: string): Promise<string | null> {
    ...
    for (const [key, u] of Object.entries(allData)) {
      ...
      if (
        t?.precinct_name &&
        t.precinct_name.toLowerCase().replace(/\s+/g, " ") === compact
      ) {
        return key;
      }
   ...
   */
  async getUnifiedPrecincts(
    precinctNames: string[],
  ): Promise<UnifiedPrecinct[]> {
    const allData = await this.getUnifiedPrecinctData();
  ...
  private async buildPrecinctDataFileFormat(): Promise<{
    ...
    const jurisdictionsMap = new Map<
      string,
      { name: string; type: "city" | "township"; precinctIds: string[] }
    >();
    for (const [name, p] of Object.entries(unifiedPrecincts)) {
      ...
      if (jurisdiction) {
        ...
        const type: "city" | "township" = jurisdiction
          .toLowerCase()
          .includes("township")
          ? "township"
          : "city";
    ...
    for (const [name, p] of Object.entries(unifiedPrecincts)) {
      ...
      precincts[name] = {
        ...
        targeting: {
          ...
          ...(p.targeting.gotvClassification != null &&
          p.targeting.gotvClassification !== ""
    ...
    const jurisdictions = Array.from(jurisdictionsMap.entries()).map(
      ([id, data]) => ({
        id,
        ...data,
      }),
    ...
    return {
      metadata: {
        ...
        dataYear: 2024,
        description:
          "Unified precinct data (targeting + optional political scores)",
        sources: [
          "precinct_targeting_scores",
          "optional precinct_political_scores",
        ],
      },
      jurisdictions,
      precincts,
  ...
  async getMunicipalityDataFileFormat(): Promise<MunicipalityDataFile> {
    if (!this.municipalityDataFileFormatPromise) {
      this.municipalityDataFileFormatPromise =
        this.buildMunicipalityDataFileFormat();
    }
  ...
  private async buildMunicipalityDataFileFormat(): Promise<MunicipalityDataFile> {
    ...
    const overallLean =
      sumPop > 0
        ? municipalities.reduce(
            (s, m) => s + m.partisanLean * m.population,
            0,
          ) / sumPop
        : 0;
    const avgTurnoutMeta =
      sumPop > 0
        ? municipalities.reduce((s, m) => s + m.avgTurnout * m.population, 0) /
          sumPop
        : 0;
    ...
    return {
      ...
      metadata: {
        ...
        dataSource:
          "Aggregated from unified precinct jurisdictions (targeting + election history)",
  ...
  private aggregatePaPrecinctGroupsToStateHouseRows(
    ...
    const wAvg = (list: any[], getter: (p: any) => number): number => {
      const totalPop = list.reduce(
        (s, p) => s + (p.demographics?.totalPopulation ?? 0),
        0,
      );
      if (totalPop <= 0) return 0;
      return (
        list.reduce(
          (acc, p) => acc + getter(p) * (p.demographics?.totalPopulation ?? 0),
          0,
        ) / totalPop
      );
    };
    ...
    const competitivenessMode = (list: any[]): string => {
      ...
      for (const p of list) {
        ...
        counts.set(
          c,
          (counts.get(c) || 0) + (p.demographics?.totalPopulation ?? 0),
        );
      }
      let best = "toss_up";
      let maxW = 0;
    ...
    for (const [districtId, { list, displayName }] of groups) {
      ...
      const avgDensity = wAvg(
        list,
        (p) => p.demographics?.populationDensity ?? 0,
      );
      ...
      for (const p of list) {
        ...
        strategyWeights.set(
          strat,
          (strategyWeights.get(strat) || 0) +
            ((p as any).demographics?.totalPopulation ?? 0),
        );
      }
      let dominantStrategy = "Low Priority";
      let maxW = 0;
      ...
      const lastElectionMargin =
        marginPop > 0
          ? Math.round((sumMarginW / marginPop) * 10) / 10
      ...
      districts.push({
        ...
        swingPotential:
          Math.round(wAvg(list, (p) => p.electoral?.swingPotential ?? 0) * 10) /
          10,
        gotvPriority:
          Math.round(wAvg(list, (p) => p.targeting?.gotvPriority ?? 0) * 10) /
          10,
        persuasionOpportunity:
          Math.round(
            wAvg(list, (p) => p.targeting?.persuasionOpportunity ?? 0) * 10,
          ) / 10,
        avgTurnout:
          Math.round(wAvg(list, (p) => p.electoral?.avgTurnout ?? 0) * 10) / 10,
        ...
        keyDemographics: {
          medianAge: Math.round(
            wAvg(list, (p) => p.demographics?.medianAge ?? 35),
          ),
          medianIncome: Math.round(
            wAvg(list, (p) => p.demographics?.medianHHI ?? 0),
          ),
          bachelorsPct:
            Math.round(
  ...
  private async buildPaDistrictChamberDataFile(
    ...
    const overallLean =
      sumPop > 0
        ? districts.reduce((s, d) => s + d.partisanLean * d.population, 0) /
          sumPop
        : 0;
    const avgTurnoutMeta =
      sumPop > 0
        ? districts.reduce((s, d) => s + d.avgTurnout * d.population, 0) /
          sumPop
        : 0;

    const competitiveDistricts = districts.filter(
      (d) => Math.abs(d.partisanLean) < 15,
    ).length;
  ...
  private async buildPaCountyDataFileFormat(): Promise<StateHouseDataFile> {
    ...
    console.log(
      `[PoliticalDataService] getPaCountyDataFileFormat: ${districts.length} counties`,
    );
    ...
    return {
      ...
      metadata: {
        ...
        competitiveDistricts,
        dataSource:
          "PA precinct aggregates + county FIPS from precinct UNIQUE_ID",
        lastElectionCycle: 2024,
        nextElectionCycle: 2026,
        lastUpdated: new Date().toISOString(),
      },
  ...
  private async buildPaSchoolDistrictDataFileFormat(): Promise<StateHouseDataFile> {
      ...
      if (!groupsBare.has(dk)) {
        ...
        labelById.set(
          ...
          typeof nm === "string" && nm.trim()
            ? nm.trim()
            : formatPoliticalDistrictLabel(dk),
        );
      }
      groupsBare.get(dk)!.push(p);
    }
    ...
    console.log(
      `[PoliticalDataService] getPaSchoolDistrictDataFileFormat: ${districts.length} districts`,
    );
    ...
    return {
      ...
      metadata: {
        ...
        competitiveDistricts,
        dataSource:
          "PA precinct aggregates + pa_precinct_district_crosswalk (schoolDistrict)",
        lastElectionCycle: 2024,
        nextElectionCycle: 2026,
        lastUpdated: new Date().toISOString(),
      },
  ...
  private async buildPaZipCodeDataFileFormat(): Promise<StateHouseDataFile> {
    ...
    console.log(
      `[PoliticalDataService] getPaZipCodeDataFileFormat: ${districts.length} ZIP areas`,
    );
    ...
    return {
      ...
      metadata: {
        ...
        competitiveDistricts,
        dataSource:
          "PA precinct aggregates + pa_precinct_district_crosswalk (zcta)",
        lastElectionCycle: 2024,
        nextElectionCycle: 2026,
        lastUpdated: new Date().toISOString(),
      },
   ...
   */
  private async mergeElectionHistoryIntoPrecinctRecords(
    precincts: Record<string, any>,
  ): Promise<void> {
    ...
    for (const [canonicalPrecinctKey, p] of Object.entries(precincts)) {
      ...
      if (isPA && p.elections && Object.keys(p.elections).length > 0) {
        ...
        if (turnouts.length > 0 && p.electoral) {
          p.electoral.avgTurnout =
            turnouts.reduce((a, b) => a + b, 0) / turnouts.length;
        }
        if (
          p.electoral &&
          ev["2024"]?.turnout != null &&
          ev["2022"]?.turnout != null
        ) {
          p.electoral.turnoutDropoff =
            ev["2024"].turnout! - ev["2022"].turnout!;
        }
      }
    }
  }
  ...
  async getSegmentEnginePrecincts(): Promise<any> {
    ...
    const precincts = Object.entries(data.precincts).map(
      ([canonicalPrecinctKey, p]: [string, any]) => {
        ...
        if (
          ...
          if (inferred) {
            if (!result.tapestryUrbanization)
              result.tapestryUrbanization = inferred.urbanization;
            if (!result.tapestryLifestage)
              result.tapestryLifestage = inferred.lifestage;
            if (!result.tapestryAffluence)
              result.tapestryAffluence = inferred.affluence;
          }
        }
        ...
        if (
          ...
          result.tapestryExpectedPartisanLean = Math.max(
            -50,
            Math.min(50, result.electoral.partisanLean),
          );
        }
    ...
    console.log(
      `[PoliticalDataService] getSegmentEnginePrecincts: Returning ${precincts.length} precincts with district and election data`,
    );
  ...
  private inferTapestryFromDemographics(p: {
    demographics?: {
      populationDensity?: number;
      medianAge?: number;
      medianHHI?: number;
    ...
    let affluence: "high" | "upper_middle" | "middle" | "modest" | "low" =
      "middle";
  ...
  private async loadTapestrySegmentsMap(): Promise<
    Map<
      string,
      {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        const filePath = path.join(
          process.cwd(),
          "public/data/tapestry/tapestry_segments.json",
        );
  ...
  > {
    ...
    for (const [dateKey, day] of Object.entries(raw.elections)) {
      ...
      out[year] = {
        ...
        turnout,
        ballotsCast:
          typeof day.ballots_cast === "number" ? day.ballots_cast : total,
      };
    }
    return out;
  }
  ...
  private matchTapestrySegmentFromInferred(
    ...
    let best: { code: string; lifeModeGroup: number; score: number } | null =
      null;
    for (const [code, seg] of map.entries()) {
      ...
      if (!best || score > best.score)
        best = { code, lifeModeGroup: seg.lifeModeGroup, score };
    }
    return best && best.score > 0
      ? { code: best.code, lifeModeGroup: best.lifeModeGroup }
      : null;
  }
  ...
  private buildSyntheticPaEngagement(p: {
    ...
    const lean =
      typeof p.electoral?.partisanLean === "number"
        ? p.electoral.partisanLean
        : 0;
   ...
   */
  private buildEngagementFromTargeting(
    targetingData: any,
  ): UnifiedPrecinct["engagement"] | undefined {
    if (!targetingData?.population_age_18up) return undefined;
    const pop18 = Number(targetingData.population_age_18up) || 1;
    const n = (key: string) =>
      Number((targetingData as Record<string, number>)[key]) || 0;
    const pct = (count: number) => (count / pop18) * 100;

    const cnnMsnbc = pct(
      n("watched_cnn_cable_news_network_last_week") +
        n("watched_msnbc_last_wk"),
    );
    const foxNewsmax = pct(
      n("watched_fox_news_channel_last_wk") + n("watched_newsmax_tv_last_wk"),
    );
  ...
  async getPrecinctElectionHistory(precinctName: string): Promise<any | null> {
    ...
    return (
      cache.electionResults?.precincts?.[precinctName] ||
   ...
   */
  async getPrecinctBlockGroups(
    precinctName: string,
  ): Promise<CrosswalkEntry[]> {
    await this.initialize();
    return (
      cache.crosswalk?.filter((entry) => entry.precinctName === precinctName) ||
      []
    );
  }
   ...
   */
  async getPrecinctDemographics(
    precinctName: string,
  ): Promise<DemographicSummary | null> {
    await this.initialize();
   ...
   */
  async getPrecinctPoliticalAttitudes(
    precinctName: string,
  ): Promise<PoliticalAttitudes | null> {
    await this.initialize();
    ...
    for (const entry of blockGroups) {
      ...
      weighted.somewhatConservative +=
        (bgData.somewhatConservative || 0) * weight;
      weighted.veryConservative += (bgData.veryConservative || 0) * weight;
      weighted.registeredDemocrat += (bgData.registeredDemocrat || 0) * weight;
      weighted.registeredRepublican +=
        (bgData.registeredRepublican || 0) * weight;
      weighted.registeredIndependent +=
        (bgData.registeredIndependent || 0) * weight;
   ...
   */
  async getPrecinctEngagement(
    precinctName: string,
  ): Promise<PoliticalEngagement | null> {
    await this.initialize();
    ...
    for (const entry of blockGroups) {
      ...
      weighted.politicalPodcastListeners +=
        (bgData.politicalPodcast || 0) * weight;
      weighted.politicalContributors +=
        (bgData.politicalContributor || 0) * weight;
      weighted.wroteCalledPolitician +=
        (bgData.wroteCalledPolitician || 0) * weight;
      weighted.cashGiftsToPolitical +=
        (bgData.cashGiftsPolitical || 0) * weight;
      weighted.followsPoliticiansOnSocial +=
        (bgData.followsPoliticians || 0) * weight;
      weighted.followsPoliticalGroups +=
        (bgData.followsPoliticalGroups || 0) * weight;
    ...
    return {
      politicalPodcastListeners:
        weighted.politicalPodcastListeners / totalWeight,
      ...
      followsPoliticiansOnSocial:
        weighted.followsPoliticiansOnSocial / totalWeight,
  ...
  async getAnalysisUnit(precinctName: string): Promise<AnalysisUnit | null> {
    ...
    const [scores, demographics, attitudes, engagement, elections, centroid] =
      await Promise.all([
  ...
  private transformRawScores(
    precinctName: string,
    raw: RawPrecinctScore,
  ): PrecinctPoliticalScores | null {
    ...
    return {
      ...
      partisanLean: {
        ...
        classification: raw.classification
          .competitiveness as CompetitivenessRating,
        electionsAnalyzed: raw.elections_analyzed,
        confidence:
          raw.elections_analyzed >= 3
            ? 0.9
            : raw.elections_analyzed >= 2
              ? 0.7
              : 0.5,
      },
      swingPotential: {
      ...
      targetingPriority: raw.classification
        .targeting_priority as TargetingPriority,
      lastUpdated: cache.politicalScores?.generated || new Date().toISOString(),
    };
  }

  private matchesFilters(
    scores: PrecinctPoliticalScores,
    filters: PoliticalFilters,
  ): boolean {
    // Partisan lean range
    if (
      filters.partisanLeanMin !== undefined &&
      scores.partisanLean.value < filters.partisanLeanMin
    ) {
      return false;
    }
    if (
      filters.partisanLeanMax !== undefined &&
      scores.partisanLean.value > filters.partisanLeanMax
    ) {
      return false;
    }
    ...
    if (
      filters.swingPotentialMin !== undefined &&
      scores.swingPotential.value < filters.swingPotentialMin
    ) {
      return false;
    }
    if (
      filters.swingPotentialMax !== undefined &&
      scores.swingPotential.value > filters.swingPotentialMax
    ) {
      return false;
    }
    ...
    // Turnout range
    if (
      filters.turnoutMin !== undefined &&
      scores.turnout.averageTurnout < filters.turnoutMin
    ) {
      return false;
    }
    if (
      filters.turnoutMax !== undefined &&
      scores.turnout.averageTurnout > filters.turnoutMax
    ) {
      return false;
    }
    ...
    // Categorical filters
    if (
      filters.competitiveness?.length &&
      !filters.competitiveness.includes(scores.partisanLean.classification)
    ) {
      return false;
    }
    if (
      filters.volatility?.length &&
      !filters.volatility.includes(scores.swingPotential.classification)
    ) {
      return false;
    }
    if (
      filters.targetingPriority?.length &&
      !filters.targetingPriority.includes(scores.targetingPriority)
    ) {
      return false;
    }
  ...
  private calculateGOTVPriority(scores: PrecinctPoliticalScores): number {
    ...
    else if (scores.turnout.dropoff && scores.turnout.dropoff > 10)
      priority += 10;

    return Math.max(0, Math.min(100, priority));
  }

  private classifyUrbanRural(
    population: number,
  ...
  private calculateRange(arr: number[]): {
    min: number;
    max: number;
    mean: number;
    median: number;
   ...
   */
  async getJurisdictionLabelForPrecinctKey(
    precinctKey: string,
  ): Promise<string | null> {
    await this.initialize();
    ...
    for (const feature of boundaries.features) {
      ...
      const longName =
        props?.Precinct_Long_Name != null
          ? String(props.Precinct_Long_Name)
          : "";
      const shortName =
        props?.Precinct_Short_Name != null
          ? String(props.Precinct_Short_Name)
          : "";
  ...
  async getPartisanLeanByIncomeBucketsForPrecinctNames(
    ...
    for (const name of precinctNames) {
      ...
      const lean =
        raw?.partisan_lean ??
        (t as { political_scores?: { partisan_lean?: number } })
          ?.political_scores?.partisan_lean ??
        null;
  ...
  async getPartisanLeanByIncomeBucketsForJurisdiction(
    ...
    return this.getPartisanLeanByIncomeBucketsForPrecinctNames(
      names,
      jurisdictionName,
    );
  }
   ...
   */
  async getJurisdictionAggregate(
    jurisdictionName: string,
  ): Promise<JurisdictionAggregate | null> {
    await this.initialize();
    ...
    for (const precinctName of precincts) {
      ...
      // Political scores
      if (
        political?.partisan_lean !== null &&
        political?.partisan_lean !== undefined
      ) {
        weightedLean += political.partisan_lean * voters;
      }
      if (
        political?.swing_potential !== null &&
        political?.swing_potential !== undefined
      ) {
      ...
      if (targeting) {
        ...
        if (targeting.targeting_strategy) {
          strategies[targeting.targeting_strategy] =
            (strategies[targeting.targeting_strategy] || 0) + 1;
        }
      }
    }
    ...
    const dominantCompetitiveness =
      Object.entries(competitivenessCount).sort(
        (a, b) => b[1] - a[1],
      )[0]?.[0] || "Unknown";
   ...
   */
  async getPrecinctsByJurisdiction(
    jurisdictionName: string,
  ): Promise<string[]> {
    await this.initialize();
  ...
  async compareJurisdictions(
    ...
    return {
      ...
      differences: {
        ...
        persuasionOpportunity:
          agg1.scores.persuasionOpportunity - agg2.scores.persuasionOpportunity,
  ...
  async rankJurisdictionsByMetric(
    ...
    // Sort by value
    rankings.sort((a, b) =>
      order === "highest" ? b.value - a.value : a.value - b.value,
    );
    ...
    for (const precinctName of precincts) {
      ...
      if (value !== null) {
        rankings.push({
          ...
          competitiveness:
            political?.classification?.competitiveness || "Unknown",
        });
      }
    }
  ...
  async rankPrecinctsStatewide(
    ...
    for (const precinctName of names) {
      ...
      switch (metric) {
        ...
        case "swing_potential":
          value =
            political?.swing_potential ?? targeting?.swing_potential ?? null;
          break;
        case "gotv_priority":
          value = targeting?.gotv_priority ?? null;
          break;
   ...
   */
  async getTopPrecinctsByMultiYearMarginSwing(
    limit: number = 15,
  ): Promise<PrecinctElectionShiftRank[]> {
    await this.initialize();
    ...
    if (isPA && data?.precincts) {
      for (const [precinctKey, row] of Object.entries(data.precincts)) {
        ...
        const cumulativeAbsMarginSwing =
          Math.abs(m22 - m20) + Math.abs(m24 - m22);
        rows.push({
          precinctName: precinctKey,
          margin2020: Math.round(m20 * 10) / 10,
          margin2022: Math.round(m22 * 10) / 10,
          margin2024: Math.round(m24 * 10) / 10,
          cumulativeAbsMarginSwing:
            Math.round(cumulativeAbsMarginSwing * 10) / 10,
      ...
      for (const [precinctId, hist] of Object.entries(data.precinctHistory)) {
        ...
        if (
          [m20, m22, m24].some(
            (m) => m === undefined || m === null || Number.isNaN(m),
          )
        ) {
          continue;
        }
    ...
    rows.sort(
      (a, b) => b.cumulativeAbsMarginSwing - a.cumulativeAbsMarginSwing,
    );
   ...
   */
  async getTurnoutTrendExtremes(
    limitEach: number = 10,
  ): Promise<TurnoutTrendExtremesResult | null> {
    await this.initialize();
    const data = await this.loadElectionResults();
    ...
    if (isPA && data?.precincts) {
      for (const [precinctKey, row] of Object.entries(data.precincts)) {
        ...
        const cumulativeAbsTurnoutSwing =
          Math.abs(t22 - t20) + Math.abs(t24 - t22);
        rows.push({
          precinctName: precinctKey,
          ...
          cumulativeAbsTurnoutSwing:
            Math.round(cumulativeAbsTurnoutSwing * 10) / 10,
        });
      }
    } else if (data?.precinctHistory) {
      for (const [precinctId, hist] of Object.entries(data.precinctHistory)) {
   ...
   */
  async getTopPrecinctsByCanvassingEfficiencyProxy(
    limit: number = 15,
  ): Promise<PrecinctCanvassingEfficiencyRank[]> {
    await this.initialize();
    const targeting = cache.targetingScores?.precincts || {};
    ...
    rows.sort(
      (a, b) => a.doorsPerPersuadableVoter - b.doorsPerPersuadableVoter,
    );
    return rows.slice(0, Math.max(1, limit));
  }

  private generateComparisonSummary(
    agg1: JurisdictionAggregate,
    agg2: JurisdictionAggregate,
  ): string {
    ...
    if (Math.abs(leanDiff) < 5) {
      parts.push(
        `${agg1.jurisdictionName} and ${agg2.jurisdictionName} have similar partisan leans`,
      );
    } else if (leanDiff > 0) {
      parts.push(
        `${agg1.jurisdictionName} leans ${Math.abs(leanDiff).toFixed(1)} points more Democratic`,
      );
    } else {
      parts.push(
        `${agg2.jurisdictionName} leans ${Math.abs(leanDiff).toFixed(1)} points more Democratic`,
      );
    }
    ...
    if (Math.abs(turnoutDiff) > 5) {
      const higher =
        turnoutDiff > 0 ? agg1.jurisdictionName : agg2.jurisdictionName;
      parts.push(
        `${higher} has higher turnout (${Math.abs(turnoutDiff).toFixed(1)}% difference)`,
      );
    }
   ...
   */
  private districtCrosswalk: Record<
    string,
    {
      precinctId: string;
      precinctName: string;
  ...
  async loadDistrictCrosswalk(): Promise<Record<string, any>> {
    ...
    try {
      const data = await fetchFromBlobOrLocal<{
        precincts?: Record<string, any>;
      }>(BLOB_KEYS.districtCrosswalk, LOCAL_PATHS.districtCrosswalk);
      ...
      console.warn(
        "[PoliticalDataService] Failed to load district crosswalk:",
        error,
      );
    }
   ...
   */
  async getPrecinctsByStateHouseDistrict(
    districtId: string,
  ): Promise<UnifiedPrecinct[]> {
    await this.initialize();
    const crosswalk = await this.loadDistrictCrosswalk();
    ...
    console.log(
      `[PoliticalDataService] Found ${matchingPrecincts.length} precincts in ${normalizedId}`,
    );
   ...
   */
  async getPrecinctsByStateSenateDistrict(
    districtId: string,
  ): Promise<UnifiedPrecinct[]> {
    await this.initialize();
    const crosswalk = await this.loadDistrictCrosswalk();
   ...
   */
  async getPrecinctsByCongressionalDistrict(
    districtId: string,
  ): Promise<UnifiedPrecinct[]> {
    await this.initialize();
    const crosswalk = await this.loadDistrictCrosswalk();
   ...
   */
  async getPrecinctsBySchoolDistrict(
    districtId: string,
  ): Promise<UnifiedPrecinct[]> {
    await this.initialize();
    const crosswalk = await this.loadDistrictCrosswalk();
    ...
    for (const [precinctName, assignment] of Object.entries(crosswalk)) {
      ...
      if (
        schoolDistrict.toLowerCase().includes(normalizedSearch) ||
    ...
    console.log(
      `[PoliticalDataService] Found ${matchingPrecincts.length} precincts in school district matching "${districtId}"`,
    );
    return matchingPrecincts;
  }
  ...
  async getDistrictAggregate(
    ...
    for (const precinct of precincts) {
      ...
      weightedPersuasion +=
        (precinct.targeting.persuasionOpportunity || 0) * pop;
    ...
    return {
      ...
      scores: {
        ...
        swingPotential:
          totalPopulation > 0 ? weightedSwing / totalPopulation : 0,
        gotvPriority: totalPopulation > 0 ? weightedGotv / totalPopulation : 0,
        persuasionOpportunity:
          totalPopulation > 0 ? weightedPersuasion / totalPopulation : 0,
