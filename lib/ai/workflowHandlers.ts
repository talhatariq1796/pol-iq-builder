...
import type {
  MapCommand,
  SuggestedAction,
} from "@/components/ai-native/AIPoliticalSessionHost";
...
import { frameAsDiscovery } from "@/lib/ai/insights";
...
import { getCitationService, type CitationKey } from "@/lib/ai/confidence";
import {
...
export async function handleDistrictAnalysis(districtParams: {
  ...
  try {
    ...
    if (!districtType || !districtId) {
      if (isPAPoliticalRegion()) {
        return {
          ...
          suggestedActions: [
            {
              id: "ph-171",
              label: "State House 171",
              action: "Show State House 171",
              icon: "map-pin",
            },
            {
              id: "ps-45",
              label: "Senate District 45",
              action: "Show Senate District 45",
              icon: "map-pin",
            },
            {
              id: "pa-07",
              label: "PA-07 Congressional",
              action: "Analyze PA-07",
              icon: "map-pin",
            },
          ],
        };
      ...
      return {
        ...
        suggestedActions: [
          ...
          {
            id: "hd-74",
            label: "State House 74",
            action: "Show State House 74",
            icon: "map-pin",
          },
          {
            id: "sd-21",
          ...
          {
            id: "mi-07",
            label: "MI-07 Congressional",
            action: "Analyze MI-07",
            icon: "map-pin",
          },
        ],
      };
    ...
    let precincts: Awaited<
      ReturnType<typeof politicalDataService.getPrecinctsByStateHouseDistrict>
    > = [];
    let districtLabel = "";
    ...
    if (districtType === "congressional") {
      precincts =
        await politicalDataService.getPrecinctsByCongressionalDistrict(
          districtId,
        );
      ...
      if (!isPAPoliticalRegion()) {
        try {
          ...
          console.warn(
            "[handleDistrictAnalysis] Could not load congressional context:",
            e,
          );
        }
      }
      ...
      precincts =
        await politicalDataService.getPrecinctsByStateSenateDistrict(
          districtId,
        );
      districtLabel = formatPoliticalDistrictLabel(districtId);
      if (!isPAPoliticalRegion()) {
        const distNum = stripDistrictIdForEnrichment(
          districtId,
          "state_senate",
        );
        try {
          ...
          console.warn(
            "[handleDistrictAnalysis] Could not load state senate context:",
            e,
          );
        }
      }
      ...
      if (!isPAPoliticalRegion()) {
        ...
        try {
          ...
          console.warn(
            "[handleDistrictAnalysis] Could not load state house context:",
            e,
          );
        }
      }
      ...
      precincts =
        await politicalDataService.getPrecinctsBySchoolDistrict(districtId);
    ...
    try {
      ...
      enrichmentContext = await enrichDistrictAnalysis(
        enrichDistrictType,
        distNum,
      );
    ...
    const getCompetitivenessLabel = (
      lean: number,
    ): { label: string; emoji: string; analysis: string } => {
    ...
    const getSwingAnalysis = (swing: number): string => {
      if (swing >= 75)
        return "High volatility — outcomes here have varied significantly across recent elections.";
      if (swing >= 50)
        return "Moderate volatility — some ticket-splitting and competitive races in recent cycles.";
      if (swing >= 25)
        return "Low volatility — voters here are fairly consistent in their choices.";
    ...
    const getGOTVAnalysis = (gotv: number, turnout: number): string => {
      if (gotv >= 70 && turnout < 60)
        return `High priority for GOTV — turnout of ${turnout.toFixed(0)}% is below average with significant untapped potential.`;
      if (gotv >= 70)
        return "Strong GOTV target due to favorable demographics and room for improvement.";
      if (gotv >= 40)
        return "Moderate GOTV potential — consider targeting if resources allow.";
    ...
    if (aggregate) {
      ...
      const turnoutDisplay =
        scores.avgTurnout > 1 ? scores.avgTurnout : scores.avgTurnout * 100;
      ...
      const turnoutAssess =
        turnoutDisplay > 65
          ...
          : turnoutDisplay > 55
            ...
            : "below average";
      const swingAssess =
        scores.swingPotential >= 50 ? "competitive" : "stable";
      const gotvAssess = scores.gotvPriority >= 60 ? "high value" : "moderate";
      const persuasionAssess =
        scores.persuasionOpportunity >= 50 ? "good target" : "lower priority";
    ...
    const sourcesCollapsible = createSourcesSection([
      "elections",
      "gis",
      "demographics",
    ]);
    ...
    if (districtType === "state_house") {
      const currentNum = parseInt(
        stripDistrictIdForEnrichment(districtId, "state_house"),
        10,
      );
    ...
    return {
      ...
      suggestedActions: [
        ...
        {
          id: "browse",
          label: "Browse all precincts",
          action: "Show me all precincts",
          icon: "list",
        },
      ],
    };
...
function filterResultPrecinctLabel(p: {
  precinctName?: string;
  name?: string;
}): string {
...
export async function handleFilterRequest(
  ...
  try {
    ...
    try {
      const precinctNames =
        results.matchingPrecincts
          ?.slice(0, 10)
          .map((p: any) => filterResultPrecinctLabel(p)) || [];
      ...
      enrichmentContext = await enrichFilterQuery(
        queryDescription,
        precinctNames,
        {
    ...
    if (filterCriteria.composite === "gotv_high_turnout_low") {
      ...
      [
        ...
        "combined_score",
        "turnout",
      ].includes(filterCriteria.metric)
    ) {
      mapCommands.push({
        type: "showHeatmap",
    ...
    if (
      results.matchingPrecincts &&
      results.matchingPrecincts.length > 0 &&
      results.matchingPrecincts.length <= 10
    ) {
      const precinctKeys = results.matchingPrecincts.map((p: any) =>
        filterResultPrecinctKey(p),
      );
      mapCommands.push({
        type: "highlight",
        target: precinctKeys,
    ...
    if (
      ...
      filterCriteria.metric === "swing_potential"
    ) {
    ...
    return {
      response:
        "I encountered an error filtering precincts. Let me help you recover.",
      suggestedActions: [
        {
          id: "retry",
          label: "Try again",
  ...
  try {
    ...
    if (dataType === "demographics") {
      if (!entity) {
        ...
        const avgMedianAge =
          precincts.reduce((sum, p) => sum + p.demographics.medianAge, 0) /
          precincts.length;
        const avgMedianIncome =
          precincts.reduce((sum, p) => sum + p.demographics.medianHHI, 0) /
          precincts.length;
      ...
      const avgTurnout =
        precincts.reduce((sum, p) => sum + p.electoral.avgTurnout, 0) /
        precincts.length;
      const highTurnout = precincts.filter(
        (p) => p.electoral.avgTurnout > avgTurnout,
      ).length;
        ...
        suggestedActions: [
          {
            id: "demographics",
            label: "Demographics",
            action: "Show me demographic data for Lansing",
            icon: "users",
          },
          {
            id: "elections",
            label: "Elections",
            action: "Show me recent election results",
            icon: "vote",
          },
          {
            id: "turnout",
            label: "Turnout",
            action: "Show me turnout data",
            icon: "trending-up",
          },
        ],
      };
    ...
    return {
      response:
        "I encountered an error retrieving data. Let me help you recover.",
      suggestedActions: [
        {
          id: "retry",
          label: "Try again",
...
export async function handleOutputIntent(
  ...
  const hasChatResolvedPrecinctList =
    ...
    (getLastChatExportPrecinctIds().length > 0 ||
      getLastUserQueryForExport().trim().length > 0);
  ...
  if (precinctsExplored > 0 || hasActiveSegment) {
    const precinctCount = hasActiveSegment
      ? segmentPrecinctCount
      : precinctsExplored;
    suggestions.push({
      id: "save-segment",
      ...
      icon: "bookmark",
      description:
        "Save these precincts as a reusable segment for future analysis",
  ...
  if (
    ...
    suggestions.push({
      ...
      icon: "database",
      description:
        "Export in VAN-compatible format for Democratic campaign tools",
    });
...
export async function handleReportIntent(
  ...
  if (precinctsExplored > 0 || hasMapSelection || hasActiveSegment) {
    reportOptions.push({
      ...
      description:
        "Comprehensive 7-page analysis with demographics, elections, and AI insights",
      pages: "7 pages",
  ...
  if (hasComparisonData || (comparisonAreas && comparisonAreas.length === 2)) {
    ...
    reportOptions.push({
      ...
      description: areas
        ? `Side-by-side analysis: ${areas[0]} vs ${areas[1]}`
        : "Side-by-side analysis of two areas",
      pages: "2-4 pages",
  ...
  if (
    hasCanvassingData ||
    (hasActiveSegment && segmentPrecinctCount > 0) ||
    precinctsExplored >= 3
  ) {
    const precinctCount = hasActiveSegment
      ? segmentPrecinctCount
  ...
  if (hasDonorData || currentTool === "donors") {
    reportOptions.push({
      ...
      description:
        "Fundraising intelligence with geographic concentration and prospects",
      pages: "3-4 pages",
      action: "report:donor",
      icon: "dollar-sign",
  ...
  if (reportOptions.length === 0) {
    ...
    return {
      response:
        "I'd love to create a report for you! First, let's define the area to analyze. You can:",
      suggestedActions: generateRecoverySuggestions("selection"),
      mapCommands: [{ type: "showChoropleth", metric: "partisan_lean" }],
    };
    ...
    suggestions = buildReportSuggestions(
      reportOptions.filter((r) => r.type === reportType || r.recommended),
    );
  } else {
...
export async function handleReportHistoryRequest(): Promise<HandlerResult> {
  ...
  const regenerateActions: SuggestedAction[] = recentReports
    .slice(0, 3)
    .map((entry) => {
      const config = REPORT_TYPE_CONFIG[entry.reportType] || {
        emoji: "📄",
...
export async function handleReportCustomization(
      ...
      suggestedActions: [
        {
          ...
          action: "Generate a precinct profile report",
          icon: "file-text",
        },
        {
          id: "comparison",
          label: "Comparison",
          action: "Generate a comparison report",
          icon: "git-compare",
        },
        {
          id: "help",
  ...
  const excludeSuggestions: SuggestedAction[] = optionalSections
    .slice(0, 3)
    .map((section) => ({
...
async function checkAndAppendSerendipitousInsight(
  ...
  try {
    ...
    console.warn(
      "[workflowHandlers] Serendipitous insight check failed:",
      error,
    );
  }
  return baseResponse;
...
function getFilterMetricDisplayValue(p: any, criteria: any): number | null {
  ...
  if (criteria.metric === "persuasion_opportunity") {
    return (
      p.persuasionOpportunity ?? p.targeting?.persuasionOpportunity ?? null
    );
  }
  if (
...
function formatFilterResults(criteria: any, results: any): string {
  ...
  if (count < 5) {
    ...
    methodologyNote =
      "May benefit from additional filtering for resource efficiency";
  } else {
  ...
  const recommendation = expressConfidence(
    confidence,
    insightText,
    methodologyNote,
  );
  ...
  if (expertise === "power_user") {
    ...
    const leadPrefix = isTightPresidentialMargin
      ...
      : `**${recommendation}**`;
    let response = isTightPresidentialMargin
      ? `${leadPrefix}\n`
    ...
    const jurisdictions = matchingPrecincts.map(
      (p: any) => p.jurisdiction || p.jurisdictionType || "Unknown",
    );
  ...
  return addCitationsToResponse(fullResponse, [
    "ELECTIONS",
    "SEGMENT_METHODOLOGY",
    "TARGETING",
  ]);
...
function addCitationsToResponse(
  ...
  const allCitations = [
    ...new Set([...explicitCitations, ...suggestedCitations]),
  ];
...
function getTargetedResponse(
  ...
  const relevantSuggestion = suggestions.find((s) => {
    if (targetType === "analysis")
      return s.id === "generate-report" || s.id === "export-csv";
...
function checkDonorGOTVOverlap(): {
  hasOverlap: boolean;
  insight: string;
  ...
  try {
    ...
    if (gotvPrecincts.length > 0 && donorData.length > 0) {
      ...
      return {
        hasOverlap: true,
        insight:
          `💡 **Cross-Tool Discovery**: You've explored ${uniqueGotvPrecincts.length} high-GOTV precincts ` +
...
function checkSegmentCanvassOpportunity(): {
  ...
  try {
    ...
    if (savedSegments.length > 0 && !hasVisitedCanvass) {
      ...
      return {
        hasOpportunity: true,
        insight:
          `💡 **Next Step Suggestion**: You've saved "${recentSegment.name}" as a segment. ` +
...
function generateFilterSoWhat(results: {
...
function generateRecoverySuggestions(
  missingContext: "selection" | "segment" | "comparison" | "filter",
): SuggestedAction[] {
  ...
  const baseActions: SuggestedAction[] = [
    {
      id: "click-map",
      label: "Click map to select",
      action: "Click on the map to select a precinct",
      icon: "map-pin",
    },
    {
      id: "browse-all",
      label: "Browse all precincts",
      action: browsePrecinctsAction,
      icon: "list",
    },
  ];
  ...
  switch (missingContext) {
    case "selection":
      return [
        ...
        {
          id: "draw-boundary",
          label: "Draw custom area",
          action: "map:enableDraw",
          icon: "edit",
        },
        {
          id: "use-saved",
          label: "Use saved segment",
          action: "navigate:segments",
          icon: "bookmark",
        },
      ];
    case "segment":
      return [
        ...
        {
          id: "quick-filter",
          label: "Quick filter: High GOTV",
          action: "Find precincts with GOTV priority above 70",
          icon: "filter",
        },
        ...baseActions,
      ];
    case "comparison":
      return [
        {
          id: "select-first",
          label: "Select first precinct",
          action: "Click on a precinct to start comparison",
          icon: "target",
        },
        {
          id: "compare-example",
    ...
    case "filter":
      return [
        {
          id: "show-filters",
          label: "Show available filters",
          action: "What filters can I use?",
          icon: "filter",
        },
        {
          id: "preset-gotv",
          label: "Use GOTV preset",
          action: "Apply GOTV targeting filter",
          icon: "zap",
        },
        {
          id: "preset-swing",
          label: "Use swing preset",
...
function getContextAwareSuggestions(
  ...
  if (context.recentPrecincts.length >= 2 && currentPrecinct) {
    const otherPrecinct = context.recentPrecincts.find(
      (p) => p !== currentPrecinct,
    );
    if (otherPrecinct) {
      suggestions.push({
    ...
    for (const outputSugg of outputSuggestions) {
      if (!existingIds.has(outputSugg.id)) {
        suggestions.push({
          ...
          icon:
            outputSugg.category === "canvassing"
              ? "map-pin"
              : outputSugg.category === "reporting"
                ? "file-text"
                : outputSugg.category === "segmentation"
                  ? "filter"
                  : "arrow-right",
  ...
  if (
    context.explorationDepth > 60 &&
    !suggestions.some((s) => s.id.startsWith("output-"))
  ) {
    suggestions.push({
      id: "save-segment",
  ...
  if (
    context.comparisonsMade >= 2 &&
    !suggestions.some((s) => s.id === "output-save-comparison")
  ) {
    suggestions.push({
...
function assessConfidence(factors: {
  ...
  if (factors.historicalElections && factors.historicalElections >= 3)
    score += 2;
  else if (factors.historicalElections && factors.historicalElections >= 2)
    score += 1;

  if (factors.margin !== undefined) {
    if (Math.abs(factors.margin) > 10)
      score += 2; // Clear trend
...
function getContextualReportResponse(options: ReportOption[]): string {
  // Group options by category
  const quickReports = options.filter((o) =>
    ["executive", "targeting"].includes(o.type),
  );
  const detailedReports = options.filter((o) =>
    ["profile", "comparison"].includes(o.type),
  );
  const operationalReports = options.filter((o) =>
    ["segment", "canvass", "donor"].includes(o.type),
  );
