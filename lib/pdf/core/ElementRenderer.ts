...
export class ElementRenderer {
  ...
  renderElements(elements: Element[]): void {
    ...
    elements.forEach((element, index) => {
      try {
        ...
        console.error(
          `[ElementRenderer] Error rendering element ${index}:`,
          error,
          element,
        );
  ...
  private renderText(element: TextElement): void {
    const {
      ...
      maxWidth,
      truncate,
      smartTruncate: useSmartTruncate,
    } = element;

    // Apply style with proper font weight mapping
    const fontName = style
      ? this.normalizeFontName(style.font || "helvetica")
      : "helvetica";
    const fontStyle = style
      ? this.normalizeFontWeight(style.weight || "normal")
      : "normal";
  ...
  private renderTextBlock(element: TextBlockElement): void {
    ...
    if (autoFit && height) {
      ...
      if (height && totalHeight > height) {
        console.warn(
          `[ElementRenderer] Text overflow detected at y=${y}: content height ${totalHeight.toFixed(2)}mm exceeds max height ${height}mm`,
        );
      }
    ...
    if (truncate && maxLines) {
      ...
      const linesArray = Array.isArray(wrappedText)
        ? wrappedText
        : [wrappedText];
    ...
    if (height && calculatedHeight > height) {
      console.warn(
        `[ElementRenderer] Text block overflow at y=${y}: ${linesToRender.length} lines (${calculatedHeight.toFixed(2)}mm) exceeds max height ${height}mm`,
      );
      ...
      if (maxLinesForHeight > 0 && linesToRender.length > maxLinesForHeight) {
        ...
        const lastLine = linesToRender[linesToRender.length - 1];
        linesToRender[linesToRender.length - 1] =
          lastLine.substring(0, Math.max(0, lastLine.length - 3)) + "...";
      }
    }
    ...
    if (contentEndY > PAGE_CONSTRAINTS.WARNING_THRESHOLD) {
      console.warn(
        `[ElementRenderer] Page overflow warning: content extends to y=${contentEndY.toFixed(2)}mm (page limit: ${PAGE_CONSTRAINTS.TOTAL_HEIGHT}mm)`,
      );
    }
  ...
  private renderIcon(element: any): void {
    ...
    if (element.name) {
      try {
        ...
        console.warn(
          `[ElementRenderer] Failed to render icon ${element.name}, using fallback:`,
          error,
        );
  ...
  private renderRadialGauge(element: any): void {
    ...
    const cx =
      typeof center === "object" && "x" in center ? center.x : center[0];
    const cy =
      typeof center === "object" && "y" in center ? center.y : center[1];
  ...
  private drawArc(
    ...
    const segments = Math.max(
      8,
      Math.ceil(Math.abs(endAngle - startAngle) / 15),
    );
    ...
    if (valueLabel) {
      ...
      if (valueLabelColorValue && valueLabelColorValue !== "transparent") {
        ...
        if (valueLabelColor !== null) {
          ...
          this.pdf.setTextColor(r, g, b);
          this.pdf.setFontSize(8);
          this.pdf.text(valueLabel, x + width, y + height / 2, {
            align: "right",
            baseline: "middle",
          });
        }
