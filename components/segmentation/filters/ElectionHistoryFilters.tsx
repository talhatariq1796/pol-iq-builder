...
export function ElectionHistoryFilters({
  ...
  const [localMaxDemVoteShare, setLocalMaxDemVoteShare] = useState(
    filters.maxDemVoteShare ?? 100,
  );
  const [localMinRepVoteShare, setLocalMinRepVoteShare] = useState(
    filters.minRepVoteShare ?? 0,
  );
  const [localMaxRepVoteShare, setLocalMaxRepVoteShare] = useState(
    filters.maxRepVoteShare ?? 100,
  );
  const [localMarginRange, setLocalMarginRange] = useState<[number, number]>(
    filters.marginRange ?? [-50, 50],
  );
  const [localMinTurnout, setLocalMinTurnout] = useState(
    filters.minTurnout ?? 0,
  );
  const [localMaxTurnout, setLocalMaxTurnout] = useState(
    filters.maxTurnout ?? 100,
  );
  const [localTurnoutDropoffMin, setLocalTurnoutDropoffMin] = useState(
    filters.turnoutDropoff?.min ?? 0,
  );
  const [localTurnoutDropoffMax, setLocalTurnoutDropoffMax] = useState(
    filters.turnoutDropoff?.max ?? 50,
  );
  const [localMinMarginShift, setLocalMinMarginShift] = useState(
    filters.trend?.minMarginShift ?? -20,
  );
  ...
  const debouncedUpdateMinDemVoteShare = useDebouncedCallback(
    (value: number) => {
  ...
  const debouncedUpdateMaxDemVoteShare = useDebouncedCallback(
    (value: number) => {
  ...
  const debouncedUpdateMinRepVoteShare = useDebouncedCallback(
    (value: number) => {
  ...
  const debouncedUpdateMaxRepVoteShare = useDebouncedCallback(
    (value: number) => {
  ...
  const debouncedUpdateMarginRange = useDebouncedCallback(
    (value: [number, number]) => {
  ...
  const debouncedUpdateTurnoutDropoffMin = useDebouncedCallback(
    (value: number) => {
  ...
  const debouncedUpdateTurnoutDropoffMax = useDebouncedCallback(
    (value: number) => {
  ...
  const debouncedUpdateMinMarginShift = useDebouncedCallback(
    (value: number) => {
  ...
  const debouncedUpdateMaxMarginShift = useDebouncedCallback(
    (value: number) => {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {formatPct(filters.minDemVoteShare ?? 0)} -{" "}
            {formatPct(filters.maxDemVoteShare ?? 100)}
          </span>
        </div>
        <div className="space-y-2">
          ...
          <Slider
            ...
            onValueChange={(value: number[]) =>
              updateFilter("minDemVoteShare", value[0])
            }
          />
          ...
          <Slider
            ...
            step={1}
            onValueChange={(value: number[]) =>
              updateFilter("maxDemVoteShare", value[0])
            }
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {formatPct(filters.minRepVoteShare ?? 0)} -{" "}
            {formatPct(filters.maxRepVoteShare ?? 100)}
          </span>
        </div>
        <div className="space-y-2">
          <div className="text-xs text-muted-foreground">Minimum</div>
          <Slider
            ...
            step={1}
            onValueChange={(value: number[]) =>
              updateFilter("minRepVoteShare", value[0])
            }
          />
          <div className="text-xs text-muted-foreground mt-3">Maximum</div>
          <Slider
            ...
            step={1}
            onValueChange={(value: number[]) =>
              updateFilter("maxRepVoteShare", value[0])
            }
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {formatMargin(filters.marginRange?.[0] ?? -50)} to{" "}
            {formatMargin(filters.marginRange?.[1] ?? 50)}
          </span>
        </div>
        <div className="space-y-2">
          ...
          <Slider
            ...
            step={1}
            onValueChange={(value: number[]) =>
              updateFilter("marginRange", value as [number, number])
            }
          />
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {formatPct(filters.minTurnout ?? 0)} -{" "}
            {formatPct(filters.maxTurnout ?? 100)}
          </span>
        </div>
        <div className="space-y-2">
          <div className="text-xs text-muted-foreground">Minimum</div>
          <Slider
            ...
            step={1}
            onValueChange={(value: number[]) =>
              updateFilter("minTurnout", value[0])
            }
          />
          <div className="text-xs text-muted-foreground mt-3">Maximum</div>
          <Slider
            ...
            step={1}
            onValueChange={(value: number[]) =>
              updateFilter("maxTurnout", value[0])
            }
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {filters.turnoutDropoff && (
            <span className="text-sm text-muted-foreground">
              {formatPct(filters.turnoutDropoff.min ?? 0)} -{" "}
              {formatPct(filters.turnoutDropoff.max ?? 50)}
            </span>
          )}
        </div>
        <div className="space-y-2">
