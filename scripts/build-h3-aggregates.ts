/**
 * Build <state>_h3_aggregates.json + <state>_h3_aggregates.geojson
 * H3 resolution-7 hexagonal aggregation of precinct targeting scores.
 *
 ...
 *
 * Usage:
 *   npx ts-node scripts/build-h3-aggregates.ts --state=california
 *   npx ts-node scripts/build-h3-aggregates.ts --state=pennsylvania
 */
...
function h3CellsForPrecinctFeature(
  ...
  try {
    ...
    console.warn(`[build-h3:${stateAbbr}] Precinct ${id}: polyfill failed, using centroid`, e);
  }
...
function main() {
  ...
  console.log(`\nBuilding H3 aggregates for ${cfg.stateName}…`);

  const precinctFile = abs(path.join(dataDir, cfg.precincts.file));
  const scoresFile = abs(path.join(dataDir, cfg.outputFiles.targetingScores));
  const outJson = abs(path.join(dataDir, cfg.outputFiles.h3Json));
  const outGeoJSON = abs(path.join(dataDir, cfg.outputFiles.h3GeoJSON));

  if (!fs.existsSync(precinctFile)) {
    throw new Error(`Precinct file not found: ${precinctFile}\nRun organize:data first.`);
  }
  ...
  console.log(`  ${fc.features.length.toLocaleString()} precincts, ${Object.keys(scores).length.toLocaleString()} targeting rows`);
  ...
  for (const f of fc.features) {
    processed++;
    if (processed % 1000 === 0) {
      console.log(`  …${processed.toLocaleString()}/${total.toLocaleString()} precincts`);
    }

    const id = String((f.properties as Record<string, unknown>)?.[cfg.precincts.uniqueIdField] ?? '');
    if (!id) continue;
    const row = scores[id];
    if (!row) {
      skippedNoScore++;
    ...
    const { cells, fallbackCentroid } = h3CellsForPrecinctFeature(feat, id, cfg.stateAbbr);
  ...
  console.log(`\n  H3 buckets: ${cellCount}`);
  console.log(`  Polyfill precincts: ${polyfillPrecincts}, centroid fallbacks: ${centroidFallbacks}`);
  console.log(`  Skipped (no targeting row): ${skippedNoScore}, (no geometry): ${skippedNoGeom}`);
  ...
  const outAgg = {
    metadata: {
      ...
      metrics: metricsList,
      note:
        `${cfg.stateName} statewide; H3 cells from polygonToCells (containmentOverlapping) per precinct. ` +
  ...
  fs.writeFileSync(outJson, JSON.stringify(outAgg, null, 2), 'utf-8');
  console.log(`\nWrote ${cellCount.toLocaleString()} H3 cells → ${cfg.outputFiles.h3Json}`);

  const outFc: FeatureCollection = { type: 'FeatureCollection', features };
  fs.writeFileSync(outGeoJSON, JSON.stringify(outFc), 'utf-8');
  console.log(`Wrote ${features.length.toLocaleString()} features → ${cfg.outputFiles.h3GeoJSON}`);
}
